...
def simulate(tasks, mode, budget=8, qos=None, seed=99):
    ...
    while sched < len(tasks) or q:
        ...
        if mode == "fcfs":
            if qos:
                q.sort(key=lambda t: -qos.get(t.tenant, 0))
            for i, t in enumerate(q):
                n = (
                    t.qubits
                    if t.qtype == "quantum"
                    else (t.qubits // 2 if t.qtype == "universal" else 0)
                )
                if n <= rem:
                    rem -= n
                    picked.append(i)
            ...
            for _ in range(len(q)):
                ...
                for i, t in enumerate(q):
                    ...
                    s = (
                        t.pri / 5 * 0.3
                        + (1 - done.get(t.tenant, 0) / max(mc, 1)) * 0.5
                        + min(t.wait / 10, 1) * 0.2
                    )
                    if s > bs:
                        bs, bi = s, i
                ...
                n = (
                    q[bi].qubits
                    if q[bi].qtype == "quantum"
                    else (q[bi].qubits // 2 if q[bi].qtype == "universal" else 0)
                )
                rem -= n
                picked.append(bi)
        if not picked:
        ...
        for i in sorted(picked, reverse=True):
            ...
            if rng.random() > fail_prob:
                tr.record_complete(t.tenant, t.exec_t)
                done[t.tenant] += 1
            else:
                tr.record_fail(t.tenant)
            sched += 1
        for t in q:
            t.wait += 1
    return tr
...
def main():
    ...
    for _name, w, qos, b in scenarios:
        ...
        t = simulate(list(tasks), "priority_fair", b, qos)
        pf_jains.append(t.jain_completion_fairness())
    ...
    lines.append(
        "- 在高优先级倾斜 + 资源争抢场景下，Priority-Fair 通过公平性加权显著平衡各租户完成率"
    )
