...
from reportlab.platypus import (
    ...
    PageBreak, KeepTogether, Preformatted, Flowable
)
...
def create_styles(font_name, bold_font):
    ...
    styles['cover_title'] = ParagraphStyle(
        'CoverTitle', fontName=bold_font, fontSize=28, leading=38,
    ...
    styles['cover_subtitle'] = ParagraphStyle(
        ...
        textColor=TEXT_MUTED, spaceAfter=30, alignment=TA_CENTER, wordWrap='CJK'
    )
    styles['cover_meta'] = ParagraphStyle(
        'CoverMeta', fontName=font_name, fontSize=11, leading=18,
        textColor=TEXT_BODY, alignment=TA_CENTER, wordWrap='CJK'
    )
    styles['h1'] = ParagraphStyle(
        'H1', fontName=bold_font, fontSize=20, leading=28,
        textColor=PRIMARY, spaceBefore=24, spaceAfter=10, wordWrap='CJK'
    )
    styles['h2'] = ParagraphStyle(
        'H2', fontName=bold_font, fontSize=15, leading=22,
        textColor=SECONDARY, spaceBefore=18, spaceAfter=8, wordWrap='CJK'
    )
    styles['h3'] = ParagraphStyle(
        'H3', fontName=bold_font, fontSize=12, leading=18,
        textColor=ACCENT_DARK, spaceBefore=12, spaceAfter=6, wordWrap='CJK'
    )
    ...
    styles['body_no_indent'] = ParagraphStyle(
        'BodyNoIndent', fontName=font_name, fontSize=10.5, leading=18,
        textColor=TEXT_BODY, spaceBefore=0, spaceAfter=8,
        firstLineIndent=0, alignment=TA_JUSTIFY, wordWrap='CJK'
    )
    styles['list_item'] = ParagraphStyle(
        'ListItem', fontName=font_name, fontSize=10.5, leading=17,
        textColor=TEXT_BODY, spaceBefore=2, spaceAfter=2,
        leftIndent=20, firstLineIndent=0, wordWrap='CJK'
    )
    styles['abstract'] = ParagraphStyle(
        'Abstract', fontName=font_name, fontSize=10.5, leading=18,
        textColor=TEXT_BODY, spaceBefore=6, spaceAfter=10,
        leftIndent=15, rightIndent=15, firstLineIndent=21,
        alignment=TA_JUSTIFY, wordWrap='CJK',
        backColor=HexColor('#f0f9ff'), borderPadding=12
    )
    styles['keywords'] = ParagraphStyle(
        'Keywords', fontName=bold_font, fontSize=10, leading=16,
        textColor=TEXT_MUTED, spaceBefore=4, spaceAfter=12, wordWrap='CJK'
    )
    styles['table_header'] = ParagraphStyle(
        'TableHeader', fontName=bold_font, fontSize=9.5, leading=14,
    ...
    styles['table_cell_left'] = ParagraphStyle(
        'TableCellLeft', fontName=font_name, fontSize=9, leading=14,
        textColor=TEXT_BODY, alignment=TA_LEFT, wordWrap='CJK'
    )
    styles['caption'] = ParagraphStyle(
        'Caption', fontName=font_name, fontSize=9, leading=13,
        textColor=TEXT_MUTED, alignment=TA_CENTER, spaceBefore=4, spaceAfter=10, wordWrap='CJK'
    )
    styles['note'] = ParagraphStyle(
        'Note', fontName=font_name, fontSize=9.5, leading=16,
        textColor=TEXT_MUTED, spaceBefore=6, spaceAfter=8,
        leftIndent=10, rightIndent=10, firstLineIndent=0,
        backColor=HexColor('#fffbeb'), borderPadding=8, wordWrap='CJK'
    )
    styles['ref'] = ParagraphStyle(
        'Ref', fontName=font_name, fontSize=9.5, leading=16,
        textColor=TEXT_BODY, spaceBefore=3, spaceAfter=3,
        leftIndent=18, firstLineIndent=-18, wordWrap='CJK'
    )
    styles['toc_h1'] = ParagraphStyle(
        'TOCH1', fontName=bold_font, fontSize=11, leading=20,
        textColor=PRIMARY, spaceBefore=8, spaceAfter=2, wordWrap='CJK'
    )
    styles['toc_h2'] = ParagraphStyle(
        'TOCH2', fontName=font_name, fontSize=10, leading=17,
        textColor=TEXT_BODY, spaceBefore=2, spaceAfter=1,
        leftIndent=15, wordWrap='CJK'
    )
    return styles
...
def parse_markdown(md_text, styles, content_width):
    ...
    while i < len(lines):
        ...
        if line.strip() == '---':
            ...
            story.append(ColoredDivider(content_width - 80, height=1, color=BORDER, space_before=4, space_after=8))
            i += 1
            continue
...
def build_table(rows, styles):
    ...
    if col_count <= 3:
        ...
        col_widths = [available_width * 0.15, available_width * 0.35, available_width * 0.15, available_width * 0.2, available_width * 0.15]
    elif col_count == 6:
        col_widths = [available_width * 0.1, available_width * 0.2, available_width * 0.15, available_width * 0.15, available_width * 0.2, available_width * 0.2]
    else:
        col_widths = [available_width / col_count] * col_count
...
def build_cover(styles, content_width):
    ...
    # Logo/装饰
    story.append(ColoredDivider(content_width, height=3, color=ACCENT, space_before=0, space_after=30))

    story.append(Paragraph("AI赋能量子计算任务调度系统", styles['cover_title']))
    story.append(Paragraph("技术白皮书", ParagraphStyle(
        'CoverTitle2', fontName="CJK-Bold", fontSize=22, leading=30,
        textColor=ACCENT_DARK, spaceAfter=16, alignment=TA_CENTER, wordWrap='CJK'
    )))
    story.append(Paragraph("Quantum-RL Co-optimized Intelligent Scheduling System", styles['cover_subtitle']))

    story.append(Spacer(1, 0.8 * inch))

    # 项目信息表格
    meta_data = [
        [Paragraph("<b>项 目 信 息</b>", styles['table_header']), Paragraph("", styles['table_header'])],
        ...
        [Paragraph("团队", styles['table_cell']), Paragraph("量子RL调度团队", styles['table_cell'])],
        [Paragraph("代码仓库", styles['table_cell']), Paragraph("github.com/xiabai2008/quantum-rl-scheduler", styles['table_cell'])],
        [Paragraph("开源协议", styles['table_cell']), Paragraph("MIT License", styles['table_cell'])],
    ]
    ...
    story.append(ColoredDivider(content_width, height=3, color=ACCENT, space_before=0, space_after=20))
    story.append(Paragraph(
        "本技术白皮书所有数字均来自可复现的实验结果<br/>"
        "审计脚本 scripts/ci/audit_authoritative_metrics.py 验证通过",
        ParagraphStyle('CoverFooter', fontName="CJK", fontSize=9, leading=15,
