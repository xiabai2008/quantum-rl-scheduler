...
def generate_ppt(output_path: str) -> None:
    ...
    add_content_slide(
        ...
        [
            ...
            "第二层：编译层 — PPO替代SABRE比特映射，深电路SWAP减少+52.1%(N=200, Wilcoxon p=2.30e-10，深电路子集强证据)",
    ...
    add_table_slide(
        ...
        [
            ...
            ["SJF", "748.48", "-54.6%", "短任务优先"],
            ["DQN(Random占位)", "697.40", "-57.7%", "DQN已删除，Random替代"],
            ["Greedy", "62.72", "-96.2%", "量子优先，均衡场景崩溃"],
            ["Quantum-Only", "-826.59", "-150.1%", "单一资源基线"],
        ],
        "多目标权衡（诚实披露）：PPO 综合奖励 +20.2%（vs 真实 FCFS），量子利用率 -3.3%（N=250 配对检验 p=0.090 不显著、95% CI 含 0，属外生指标噪声而非策略缺陷；未达团队基线 R-P-01 的 30% 目标，不作卖点；该目标非比赛方案明文要求）；统计协议：Welch t + Mann-Whitney U 双验证，28组Bonferroni校正（α=0.0018），95% CI [+14.3%, +26.7%]，功效分析达标，原始JSON可复现",
    )
    ...
    add_content_slide(
        ...
        [
            ...
            "  - 深电路(14-16q) SWAP减少+52.1%（N=200, Wilcoxon p=2.30e-10高度显著；全60电路p=8.40e-01不显著，诚实披露）",
            "  - 原-76.4%为不公平对比已废弃",
    ...
    add_content_slide(
        prs,
        "量子赋能AI：量子硬件数据赋能评估可信度与稳健性建模",
        [
            "核心逻辑：用量子硬件的真实测量数据校准AI评估环境（方法论贡献，非性能提升）",
            "三角证据链：",
            "  ① 机制成立：AI评估对量子噪声显著敏感（N=25配对 p=2.98e-08，-12.43%负向敏感性）→ '真机测量→评估校准'链路有效",
            "  ② 真机反馈训练影响：issue192 纯仿真训练模型评估 999.7 vs 含真机反馈训练 184.3/-298.8 → 量子数据影响 AI 训练",
            "  ③ 跨机器异质性：真机vs仿真机MBS差异显著（p=0.0197）→ 量子测量揭示仿真盲区，数据不可替代",
            "闭环流程：真机H门测量（保真度0.976）→ 噪声分布建模 → 注入仿真环境 → 评估/训练反馈",
            "诚实定位：不宣称'量子提升AI性能'——数据不支持；'评估可信度与稳健性建模赋能'由上述统计证据支撑，与榜题'研究与应用探索'契合",
        ],
    )
    ...
    add_content_slide(
        ...
        [
            ...
            "其他诚实边界（主动披露）：",
            "  - 量子利用率-3.3%（N=250配对检验p=0.090不显著，CI含0，外生指标噪声非策略缺陷；未达团队基线R-P-01的30%目标，不作卖点）",
    ...
    add_table_slide(
        ...
        ["策略", "平均奖励", "备注"],
        [
            ["PPO", "1632.26", "N=20 v3 权威（8.14 扩样；d=2.11, p=1.22e-07）"],
            ["SJF", "607.54", "N=20 v3 权威"],
            ["FCFS", "782.94", "N=20 v3 权威基线"],
        ],
        "诚实声明：真机验证主要证明SDK可用性（315次100%成功），性能数字以仿真N=250结果为准；上表为混合环境策略对比（96步中仅1步走真机，真机reward占比约1%）",
    )
    ...
    add_content_slide(
        ...
        [
            ...
            "  - Q: 利用率-3.3%怎么解释？ A: 三点：①该差值经N=250配对检验p=0.090不显著（95% CI含0）——不是策略劣势的统计证据；②该指标是环境外生随机游走（available_ratio不受调度动作控制），我们做了权重扫描（W=0.5→10）证明无法通过调参改变；③优化目标是综合调度收益（多目标权衡），等待时间-14.0%双优。如实披露正是因为实验是真的",
            "  - Q: 为什么性能验证靠仿真？ A: 真机验证两级：可用性（315次100%成功）已达成；性能级为混合环境策略对比（8.14 扩样至 N=20/组 v3 权威，d=2.11, p=1.22e-07；真机 reward 占比 1/96 步）。权威性能结论由N=250仿真+真实FCFS基线支撑，完全可复现",
