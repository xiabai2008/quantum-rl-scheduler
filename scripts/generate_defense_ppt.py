...
def generate_ppt(output_path: str) -> None:
    ...
    add_content_slide(
        ...
        [
            ...
            "第一层：调度层 — PPO智能调度实时最优分流，+88.3% vs FCFS（N=250, p=1.032e-42）",
            "第二层：编译层 — PPO替代SABRE比特映射，深电路SWAP减少~33%(n=20, p=0.29不显著)",
            "第三层：量子赋能AI — 真机噪声模型提取→仿真环境校准→PPO鲁棒性提升（主方向）",
            "双向赋能闭环：AI优化量子调度→量子真机噪声反馈优化AI→更优调度→更高利用率",
            "关键数字：PPO +88.3%、防饥饿0.60 vs 22.00、推理延迟~1ms、315次真机调用100%成功",
        ],
    )
    ...
    add_table_slide(
        prs,
        "PPO策略综合奖励领先：比FCFS提升88.3%",
        "8策略公平对比（N=250，50 seed × 5 episode）",
        ["策略", "平均奖励", "vs FCFS", "备注"],
        [
            ...
            ["Random", "1277.5", "-12.4%", "随机基线"],
        ],
        "帕累托最优权衡：PPO牺牲1.2%利用率换取88.3%总奖励提升（多目标优化合理取舍）",
    )
    ...
    add_content_slide(
        ...
        [
            ...
            "  - 深电路(14-16q) SWAP减少~33%（n=20, p=0.29不显著）",
            "  - 原76.4%为不公平对比已废弃",
    ...
    add_content_slide(
        ...
        [
            ...
            "结果：奖励提升+6.4%，p=0.190（不显著），训练时间开销+74.5%",
            "当前定位：",
    ...
    add_content_slide(
        ...
        [
            ...
            "  - PPO强化学习实现毫秒级动态决策，+88.3% vs FCFS（p=1.032e-42）",
    ...
    add_content_slide(
        ...
        [
            ...
            "  - 感谢指导老师的悉心指导",
            "预设Q&A：",
            "  - Q: 利用率为什么比FCFS低？ A: 帕累托最优权衡（牺牲1.2%利用率换88.3%奖励提升）",
            "  - Q: 量子赋能AI为什么是探索性的？ A: QUBO退火p=0.190不显著，已转向真机噪声反馈",
