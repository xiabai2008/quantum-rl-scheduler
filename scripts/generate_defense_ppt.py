...
def generate_ppt(output_path: str) -> None:
    ...
    add_content_slide(
        ...
        [
            ...
            "双向闭环：AI优化量子调度→量子真机测量数据反哺AI训练环境→更优调度决策",
    ...
    add_table_slide(
        ...
        "多目标权衡（诚实披露）：PPO 综合奖励 +20.2%（vs 真实 FCFS），量子利用率 -3.3%（未达赛题30%目标，不作卖点）；统计协议：Welch t + Mann-Whitney U 双验证，28组Bonferroni校正（α=0.0018），95% CI [+14.3%, +26.7%]，功效分析达标，原始JSON可复现",
    )
    ...
    add_content_slide(
        ...
        [
            ...
            "结果：奖励变化-5.6%，p=0.9430（20seeds权威，不显著；旧5seed +6.4%已废弃），训练时间开销+74.5%",
            "当前定位：",
