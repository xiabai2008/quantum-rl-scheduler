...
def generate_ppt(output_path: str) -> None:
    ...
    add_content_slide(
        ...
        [
            "核心思路：用AI智能调度量子资源，用量子真机噪声反馈优化AI鲁棒性",
            "第一层：调度层 — PPO智能调度实时最优分流，+123.4% vs FCFS（N=250, p=1.449e-66）",
            "第二层：编译层 — PPO编译Agent（探索性验证）比特映射，深电路SWAP减少~33%(n=20, 整体p=0.86不显著)",
            "第三层：量子赋能AI — 真机噪声模型提取→仿真环境校准→PPO鲁棒性提升（主方向）",
            "双向赋能闭环：AI优化量子调度→量子真机噪声反馈优化AI→更优调度→更高利用率",
            "关键数字：PPO +123.4%、防饥饿0.60 vs 22.00、推理延迟~1ms、315次真机调用100%成功",
        ],
    )
    ...
    add_table_slide(
        prs,
        "PPO策略综合奖励领先：比FCFS提升123.4%",
        "8策略公平对比（N=250，50 seed × 5 episode，16维交付模型）",
        ["策略", "平均奖励", "vs FCFS", "备注"],
        [
            ["PPO", "2348.91", "+123.4%", "第一名, p=1.449e-66"],
            ["SJF", "1060.30", "+0.8%", "短任务优先"],
            ["FCFS", "1051.59", "基线", "传统调度"],
            ["Random", "891.53", "-15.2%", "随机基线"],
            ["Greedy", "-134.18", "-112.8%", "量子优先，均衡场景崩溃"],
            ["Quantum-Only", "-940.56", "-189.4%", "单一资源基线"],
        ],
        "帕累托最优权衡：PPO牺牲1.2%利用率换取123.4%总奖励提升（多目标优化合理取舍）",
    )
    ...
    add_content_slide(
        ...
        [
            ...
            "验证数据：",
            "  - 保真度：~0.976（真机测量）",
            "  - 等待时间：+6.1%（10seeds，探索性，Issue #532）",
            "  - 单seed -5.7% 方向相反，待更多数据验证",
    ...
    add_content_slide(
        ...
        [
            ...
            "  - PPO强化学习实现毫秒级动态决策，+123.4% vs FCFS（p=1.449e-66）",
            "  - 三层架构：调度层+编译层+高负载防饥饿（0.60 vs 22.00）",
            "创新2：量子赋能AI（主方向：真机噪声反馈）",
            "  - 真机噪声→模型校准→PPO鲁棒性提升，形成量子→AI闭环",
            "  - 保真度~0.976，探索性结果（10seeds，待更多验证）",
            "创新3：天衍云平台全链路对接",
    ...
    add_content_slide(
        ...
        [
            ...
            "  - 感谢指导老师的悉心指导",
            "预设Q&A：",
            "  - Q: 利用率为什么比FCFS低？ A: 帕累托最优权衡（牺牲1.2%利用率换123.4%奖励提升）",
            "  - Q: 量子赋能AI为什么是探索性的？ A: QUBO退火p=0.9430不显著，已转向真机噪声反馈（N=25配对p=2.98e-08显著）",
