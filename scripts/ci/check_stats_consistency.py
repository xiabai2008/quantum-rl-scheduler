...
INTERNAL_CONFLICTS: list[tuple[list[str], str]] = [
    ...
    (
        ["+38.5%", "+52.1%"],
        "编译层深电路 +38.5%（N=80 已被 N=200 取代）与 +52.1%（N=200 权威）并存",
    ),
    (["+38.5%", "+33.3%"], "编译层深电路 +38.5%（N=80 历史）与 +33.3%（N=20 旧样本）并存"),
]
...
_DISCLOSURE_MARK = (
    ...
    "非统计证据",
    "历史",
    "取代",  # 8.14：数字升级后"已被 N=200 取代"语境属诚实披露（A8 文件级豁免）
)
...
EXCLUDE_PATTERNS = [
    ...
    # 避免门禁对其自身误报。
    "红队审查",
    # 8.14 round12 审查：历轮审查报告（round*_审查报告.md）同为内部审计产物
    # （submission_manifest.yaml 已排除入包），原文引用旧口径/孤儿 p 值属正常
    # 诚实披露，门禁应与 manifest 口径一致豁免，避免对内部审计文档误报。
    "审查报告",
    # 8.14 第三轮审查 P3：PR审核日志为内部日志（含"1.449e-66→7.56e-12"等历史替换
    # 表述），豁免避免 cosmetic 告警（与 PR审查 同源）。
    "审核日志",
]

# p值正则模式：匹配 p=0.031, p=1.032e-42, p<0.001, p=3.04×10⁻¹¹ 及中文"p 值：0.127"、
# "p 值为 1.449e-66"、"p 值是 7.56e-12"（8.14 第三轮审查 P2：补系动词"为/是"盲区）
P_VALUE_PATTERN = re.compile(
    r"(?:p\s*[=<>]\s*|p\s*值\s*(?:[:：]|为|是)?\s*)"
    r"(?:"
...
HONEST_DISCLOSURE_KEYWORDS = [
    ...
    "废弃",
    "假数据",
    "取代",  # 8.14：数字升级后"已被 N=200 取代"等语境属诚实披露，豁免黑名单/孤儿检测
]
...
BLACKLIST_PATTERNS: list[tuple[str, str]] = [
    ...
    (
        ...
        "BLACKLIST: 真机PPO均值已升级为N=20 v3权威值1632.26（8.14 扩样），旧N=5值1665.22严禁作为对外基准（若为历史引用请显式标注N=5并说明已废弃）",
    ),
    (
        r"FCFS.*真机.*353\.22|真机.*FCFS.*353\.22|353\.22.*FCFS.*真机",
        "BLACKLIST: 真机FCFS均值已升级为N=20 v3权威值782.94（8.14 扩样），旧N=5值353.22严禁作为对外基准（若为历史引用请显式标注N=5并说明已废弃）",
    ),
    ...
    # 8.13 审查补强（C 报告盲区 a）：+4.0% 为 MAPPO N=10 旧口径（8.11 已升级 N=20），
    # 此前不在黑名单中可溜过门禁；README/novelty/value/defense_qa 等多处曾残留。
    (
        r"(?<!\d)\+4\.0%(?!%)",
        "BLACKLIST: MAPPO 协同优势 +4.0% 为 N=10 旧口径（8.11 已升级 N=20）；权威为 +36.5%（同训练量收敛严格对比，N=20，50K，p=0.024）",
    ),
    (
...
_MD_STRIP_RULES: list[tuple[re.Pattern[str], str]] = [
    (re.compile(r"`[^`]*`"), " "),  # 行内代码
    # 8.11 修复：加粗/斜体/下划线保留文本仅去标记（此前整体删除导致 **+6.4%** 等
    # 加粗包裹的废弃数字逃逸黑名单检测；保留文本使数字仍可被扫描命中）
    (re.compile(r"\*\*([^*\n]+)\*\*"), r"\1"),  # 加粗 → 保留内容
    (re.compile(r"\*([^*\n]+)\*"), r"\1"),  # 斜体 → 保留内容（不成对的 * 视为乘法，不剥离）
    (re.compile(r"__([^_\n]+)__"), r"\1"),  # 加粗（alt）→ 保留内容
    (re.compile(r"\[([^\]\n]*)\]\([^)\n]*\)"), r"\1"),  # 链接保留文本
]
