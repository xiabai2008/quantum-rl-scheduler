...
# ---------------------------------------------------------------------------
# 9.5 权威数字复算门禁（8.14 round12 新增：yaml↔JSON 全量复算一致性）
# ---------------------------------------------------------------------------
echo ""
echo -e "${BOLD}[9.5/11]${NC} 权威数字复算门禁（reproduce_authoritative）..."

if python scripts/ci/reproduce_authoritative.py >/dev/null 2>&1; then
    pass "权威数字复算通过（statistics.yaml ↔ 原始 JSON 全量一致）"
else
    fail "权威数字复算失败（存在派生值残留，见上方输出）"
fi
...
check_file "deliverable_models/ppo_best_model_16dim.zip" "required"
check_file "deliverable_models/ppo_compilation_agent.zip" "required"
...
if [ "$FAIL" -gt 0 ]; then
    ...
    echo ""
    echo -e "${BOLD}下一步:${NC}"
    echo "  git tag -a v9.1-submission -m 'v9.1 提交版本 (2026-08-15)'"
    echo "  git push origin v9.1-submission"
    echo ""
