...
核心能力：
1. 扫描仓库中所有权威实验数字（PPO=1982.69 / FCFS=1648.91 / +20.2% 等），
   记录每个出现位置的文件、行号。
...
CANONICAL_METRICS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("PPO 平均奖励", re.compile(r"(?<![\d.])1982\.69(?![\d])")),
    ("FCFS 平均奖励", re.compile(r"(?<![\d.])1648\.91(?![\d])")),
    ("PPO 相对 FCFS 提升", re.compile(r"\+20\.2%")),
    ("rank-biserial 效应量", re.compile(r"(?<![\d.])-?0\.3642(?![\d])")),
    ("Welch t 检验 p 值", re.compile(r"7\.56[eE]-12")),
