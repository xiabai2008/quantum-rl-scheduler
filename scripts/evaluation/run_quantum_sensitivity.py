...
注: 本脚本使用 14 维原生环境（不使用 10 维截断包装），与项目主对比框架
（14 维原生环境, 200步, 50 seeds）配置一致，绝对奖励值可比较。
"""
...
def main():
    ...
    report_lines = [
        ...
        "",
        "已有权威数字（PPO +20.2%, p=7.56e-12）仅在量子占比 70% 这一个点测得。",
        "本实验通过 5 个梯度点（10% / 30% / 50% / 70% / 90%）系统刻画 PPO 优势曲线。",
        "",
        "> **方法论差异说明**: 本实验使用 14 维原生环境，",
        "> 与项目权威对比框架（14 维原生环境, 200步, 50 seeds）配置一致，",
        "> 绝对奖励值可比较。提升百分比在同一配置内对比有效。",
        "",
        "---",
        "",
        "## 实验结果",
    ...
    if 70 in ratios_list:
        ...
        report_lines += [
            f"- **与权威数字对比**: 70% 量子占比下测得提升 +{ref_imp:.1f}%（权威数字 +20.2%，差异来源于 seed 数/训练策略/模型版本）",
        ]
