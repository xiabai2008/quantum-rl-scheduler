...
from scripts.evaluation.run_simulation import (
    SimulationEnv, SimulationTaskGenerator, FCFSStrategy, PPOStrategy,
    ShortestJobFirstStrategy, run_strategy,
)
...
for i, mol in enumerate(MOLECULES.keys()):
...
for strategy_name, strategy in [
    ...
    r = run_strategy(sim_env, strategy, num_episodes=5, tasks_per_episode=100, max_steps=200, verbose=False)
    ...
    print(f"  {strategy_name}: reward={r['avg_reward']:.0f}, wait={r['avg_wait_time']:.1f}, "
...
for name, cfg in MOLECULES.items():
    ...
    lines.append(f"| {name} | {cfg['qubits']} | {cfg['reps']} | {cfg['shots']} | {cnt} | {cfg['priority']} |")
