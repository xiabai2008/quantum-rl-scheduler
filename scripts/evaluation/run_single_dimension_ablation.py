...
逐一剔除 14 维观测空间中的维度组，使用启发式策略评估各维度组的可利用性，
量化各维度组对调度性能的独立贡献。
...
from src.scheduler.baselines import FCFSScheduler, SPTFScheduler
...
# 基线调度器实例（FCFS / SPTF≡SJF）。
# baselines 模块策略通过 select_action(tasks, available_resources) 返回所选任务在队列
# 中的索引，需在 _baseline_action 中适配为环境的资源类型动作（见下）。
BASELINE_SCHEDULERS: dict[str, Any] = {
    "fcfs": FCFSScheduler(),
    "sjf": SPTFScheduler(),  # SPTF ≡ SJF（最短处理时间优先 = 最短作业优先）
}
...
def _baseline_action(env: gym.Env, scheduler: Any) -> int:
    """用基线调度器从环境队列选任务，并映射为资源类型动作。

    ``src.scheduler.baselines`` 的策略 ``select_action(tasks, available_resources)``
    返回所选任务在队列中的索引；而本环境动作空间为资源类型
    （0=经典 / 1=量子 / 2=混合）。这里按所选任务的类型映射为兼容的资源动作，
    队列为空或选择无效时回退到混合动作（兼容所有任务类型）。

    Note:
        环境内部按 priority/wait_steps 取出 ``_current_task`` 执行，策略选出的任务
        索引仅用于决定资源类型，二者并非同一对象——这是将“选任务”型基线适配到
        “选资源”型动作空间的启发式折中（详见报告局限性章节）。
    """
    inner = env.unwrapped
    queue = getattr(inner, "_task_queue", [])
    if not queue:
        return ACTION_HYBRID

    # Task 没有 arrival_time 字段，用 -wait_steps 近似（等待越久 = 到达越早），
    # 使 FCFS 选出最早到达的任务；estimated_time 用 execution_time 供 SJF/SPTF 排序。
    tasks_as_dicts = [
        {
            "task_id": t.task_id,
            "task_type": t.task_type,
            "qubit_count": t.qubit_count,
            "priority": t.priority,
            "estimated_time": float(t.execution_time),
            "arrival_time": -float(t.wait_steps),
            "urgency": t.urgency,
        }
        for t in queue
    ]

    quantum = getattr(inner, "_quantum", None)
    classical = getattr(inner, "_classical", None)
    available_resources: dict[str, float] = {}
    if quantum is not None:
        available_resources["qubits"] = int(
            getattr(quantum, "available_ratio", 0.0) * getattr(quantum, "total_qubits", 0)
        )
        available_resources["fidelity"] = float(getattr(quantum, "fidelity", 0.0))
    if classical is not None:
        available_resources["classical_load"] = float(getattr(classical, "load", 0.0))

    idx = scheduler.select_action(tasks_as_dicts, available_resources)
    if idx < 0 or idx >= len(queue):
        return ACTION_HYBRID

    task_type = queue[idx].task_type
...
def run_episode(
    ...
    scheduler = BASELINE_SCHEDULERS.get(strategy)

    for _ in range(max_steps):
        if strategy == "random":
            action = env.action_space.sample()
        elif scheduler is not None:
            action = _baseline_action(env, scheduler)
        else:
            action = env.action_space.sample()
...
def generate_report(all_results: list[dict], output_path: Path) -> None:
    ...
    lines = [
        ...
        "",
        "> ⚠️ 本报告为设计文档，实验使用启发式策略（非训练PPO），反映维度可利用性而非RL最终贡献",
        "",
        "## 1. 实验设计",
        "",
        "采用**逐一剔除法**（Ablation by Removal）：对每个维度组，",
        "使用启发式策略评估[去掉该维度组]的维度可利用性，对比完整 14 维基线的性能差异。",
        "",
        "| 实验组 | 剔除维度 | 保留维度 | 目的 |",
        "|:-------|:---------|:---------|:-----|",
    ]
...
def main() -> None:
    ...
    # 生成报告
    report_path = PROJECT_ROOT / "results" / "reports" / "ablation_single_dimension.md"
