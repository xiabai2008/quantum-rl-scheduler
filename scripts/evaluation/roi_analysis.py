...
def extract_real_machine_rewards(data: dict[str, Any]) -> dict[str, list[float]]:
    ...
    for entry in data.get("results", []):
        ...
        if strategy and "metrics" in entry:
            rewards.setdefault(strategy, []).append(entry["metrics"]["total_reward"])
    return rewards
...
def verify_real_machine_numbers(rewards: dict[str, list[float]]) -> bool:
    ...
    print(
        f"[验证] PPO: {ppo_mean:.2f}±{ppo_std:.2f} (权威: {REAL_PPO_MEAN}±{REAL_PPO_STD}) → {'✅' if ppo_ok else '❌'}"
    )
    print(
        f"[验证] FCFS: {fcfs_mean:.2f}±{fcfs_std:.2f} (权威: {REAL_FCFS_MEAN}±{REAL_FCFS_STD}) → {'✅' if fcfs_ok else '❌'}"
    )
...
def calc_annual_research_time_value(users: int, monthly_hours: float, hourly_rate: float) -> float:
    """年化科研时间价值 = 活跃用户 × 月省小时 × 时薪 × 12。"""
    return users * monthly_hours * hourly_rate * 12


def calc_annual_failure_saving(failure_reduction: float, task_cost: float) -> float:
...
def generate_report(
    ...
    annual_failure = calc_annual_failure_saving(params["failure_reduction"], params["task_cost"])
...
def main() -> int:
    ...
    parser.add_argument(
        "--utilization-improvement", type=float, default=DEFAULT_UTILIZATION_IMPROVEMENT
    )
    parser.add_argument("--active-users", type=int, default=DEFAULT_ACTIVE_USERS)
    parser.add_argument(
        "--monthly-hours-saved", type=float, default=DEFAULT_MONTHLY_HOURS_SAVED_PER_USER
    )
