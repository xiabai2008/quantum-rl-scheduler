...
class CircuitPool:
    def __init__(self, n=80):
        self.circuits = [
            random_circuit(np.random.randint(5, 13), np.random.randint(5, 21), measure=False)
            for _ in range(n)
        ]
        self.idx = 0
    ...
    def sample(self):
        self.idx += 1
        return self.circuits[self.idx % len(self.circuits)]
...
def cp_sat(tasks, tl=60):
    m = cp_model.CpModel()
    h = sum(max(1, t[1]) for t in tasks)
    s, e, iv = [], [], []
    for i, (_, d) in enumerate(tasks):
        si = m.NewIntVar(0, h, f"s{i}")
        ei = m.NewIntVar(0, h, f"e{i}")
        s.append(si)
        e.append(ei)
        iv.append(m.NewIntervalVar(si, d, ei, f"iv{i}"))
...
for n in [20, 50, 100]:
    ...
    ort_in = [
        (i, t.get("qubit_count", 0) * 10 + 5 if t.get("task_type") == "quantum" else 5)
    ...
    ff = run_strategy(
        se, FCFSStrategy(), num_episodes=3, tasks_per_episode=n, max_steps=n, verbose=False
    )
    print(
        f"  {n} tasks: OR-Tools makespan={ort_r}, PPO reward={pf['avg_reward']:.0f}, FCFS={ff['avg_reward']:.0f}"
    )
...
result = {
    "ppo_avg_swap": ppo_avg,
    "sabre_avg_swap": sabre_avg,
    "improvement_pct": round(improvement, 1),
    "model": "ppo_compilation_agent.zip",
}
