...
def run_single_seed(strategy: Any, seed: int) -> tuple[list[float], dict[str, Any]]:
    ...
    inner_env = QuantumSchedulingEnv(max_steps=TASKS_PER_EPISODE, max_qubits=287, seed=seed)
...
> **⚠️ 重要说明（数据一致性）**：本报告 PPO 相对 FCFS 提升 **{ppo_fcfs["improvement_pct"]:+.1f}%**（14维原生观测，{total_runs} runs），权威数字 **+20.2%**（16维交付模型，N=250，vs 真实 FCFS；旧 +123.4% 为 vs Hybrid-Default 弱基线，已废弃）存在差异。差异源于观测维度（14维 vs 16维）权样本量（{total_runs} vs 250 runs）不同；两套实验均显示 PPO 显著优于 FCFS，策略排名一致。详见 §3.3。
...
> **⚠️ 数据一致性说明**：本报告 PPO 提升 {ppo_fcfs["improvement_pct"]:+.1f}% 权威数字 +20.2% 存在差异，原因如下：
...
def main() -> None:
    ...
    print("=" * 64)
    print(
        f"  Seeds:         {NUM_SEEDS} x {EPISODES_PER_SEED} episodes = {total_runs} runs/strategy"
    )
    ...
    strategies = [
        ("PPO", PPOStrategy(ppo_model)),
        ("DQN", DQNModelStrategy(dqn_model)),
        ("FCFS", FCFSStrategy()),
    ]
    ...
    for name, strategy in strategies:
        ...
        for seed_idx in range(NUM_SEEDS):
            ...
            t0 = time.time()
            ep_rewards, summary = run_single_seed(strategy, seed)
            all_episode_rewards.extend(ep_rewards)
            all_summaries.append(summary)
            print(f"avg_reward={np.mean(ep_rewards):.2f} ({time.time() - t0:.1f}s)")
    ...
    for name in ["PPO", "DQN", "FCFS"]:
        ...
        summary_metrics[name] = {
            ...
            "mean_wait_time": round(float(np.mean([s["avg_wait_time"] for s in summaries])), 4),
