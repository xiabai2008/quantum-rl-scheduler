...
def solve_cp_sat(tasks: list, n_machines: int = 2, time_limit: int = 60):
    ...
    for i in range(n):
        ...
        model.NewIntVar(durations[i], durations[i], f"d_{i}")
        starts.append(s)
        ends.append(e)
...
def run_comparison(tasks_per_scale: list[int] | None = None, episodes: int = 5):
    """跨规模对比 PPO vs OR-Tools vs FCFS vs SJF"""
    if tasks_per_scale is None:
        tasks_per_scale = [20, 50, 100]
    ...
    for n_tasks in tasks_per_scale:
        ...
        SimulationEnv(env=env, task_generator=SimulationTaskGenerator(seed=42))
        ...
        ortools_r = solve_cp_sat(
            ortools_tasks, n_machines=2, time_limit=30 if n_tasks <= 50 else 60
        )
        ...
        print(
            f"  OR-Tools: makespan={ortools_r['makespan']}, time={ortools_r['wall_time']:.2f}s, status={ortools_r['status']}"
        )
...
def _generate_report(results: dict, ts: str):
    ...
    for n in scales:
        ...
        ort = (
            f"makespan={r['OR-Tools']['makespan']}"
            if r["OR-Tools"]["makespan"]
            else f"求解失败({r['OR-Tools']['status']})"
        )
