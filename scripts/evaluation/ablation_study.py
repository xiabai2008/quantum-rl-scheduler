...
def evaluate_model(model_path, is_14_dim, episodes=10, tasks_per_episode=200, seed=42):
    ...
    for ep in range(episodes):
        ...
        avg_wait = sum(t.wait_steps for t in raw_env._task_queue) / len(raw_env._task_queue) if raw_env._task_queue else 0
...
def main():
    ...
    model_14_path = os.path.join(PROJECT_ROOT, "deliverable_models", "ppo_best_model_14dim.zip")
    ...
    print(f"{'Mismatch Count':<20} | {f14:<15.2f} | {f16:<15.2f} | {f_imp:+.2f}% (Fewer mismatches)")
    print("=" * 70)
