...
| R-F-04 | 实现量子启发式退火优化RL训练（探索性方向） | 功能 | `src/quantum/annealing.py`<br>`src/scheduler/async_annealing_callback.py` | **已实现** | 20 Seed 消融实验 | "量子启发式退火将策略网络权重映射为 QUBO 问题（当前使用经典模拟退火D-Wave neal），20seed 权威方向为 -5.6%（p=0.9430不显著，训练开销+74.5%；旧 5seed +6.4% 已废弃）" | `results/reports/annealing_ablation_20seeds_report.md` |
| R-F-05 | 支持多机器协同调度 | 功能 | `src/scheduler/marl.py`<br>`src/scheduler/env.py` | **已实现** | 同训练量收敛严格对比 | "MAPPO 多智能体协同调度，3 台真机负载均衡 CV=0.246，同训练量收敛严格对比下协同优势 +36.5%（MAPPO vs 独立PPO，N=20，配对 Wilcoxon p=0.024；旧 +84.6% p=0.0188 为未收敛+训练量不均等的探索值、旧 +4.0% 为 N=10 口径，均已废弃）；叠加规模扩展效应后总提升 vs 单机 +86.3%" | `results/reports/mappo_strict_comparison_report.md` |
...
| R-P-05 | 压力测试：4 种极端场景稳定性 | 性能 | `scripts/benchmarking/stress_test.py` | **已达成** | 压力测试 | "PPO 在 4 种压力场景下综合稳定性最强，量子波动场景比第二名提升 91.4%（历史探索数据，诚实化前旧 FCFS 基线，权威 stress 数据待重跑核定，仅作场景适配参考）" | `results/reports/stress_test_report.md` |
...
| R-E-02 | 测试覆盖率 ≥80% | 工程 | `tests/` | **已达成** | pytest-cov | "测试覆盖率 ≥80%（CI 门禁 --cov-fail-under=80），主套件 3730 测试用例，94+ 个测试文件" | `tests/` 目录 |
...
| R-I-02 | 双向赋能：量子赋能 AI（探索性方向） | 创新 | `src/quantum/annealing.py` | **已实现** | 消融实验 | "量子退火优化RL（探索性方向，当前为经典模拟退火，训练开销+74.5%，p=0.9430不显著）；核心闭环为AI优化量子调度（RL智能调度为核心claim）" | `results/reports/ablation_report.md` |
...
|---------|---------|---------|---------------------|-------|------|
| R-P-02 | 平均等待时间降低 ≥40%（赛题原始需求目标，非声称结果） | **口径说明**：权威实测（N=250）PPO 平均等待时间 25.46 步，低于 FCFS 29.61 步（**-14.0%，PPO 更优**），未达 ≥40% 目标但优于基线，且与吞吐量 +20.2% 无明显负权衡（吞吐量与等待时间双优）。旧表述"等待时间为帕累托权衡、PPO 等待高于 FCFS、等待时间恒定 57.73 步"已被权威口径推翻，予以废弃 | **否** | 中 | 详见 `results/multiseed_evaluation/rewards_multiseed.json` 与 `statistics.yaml` <!-- audit-exempt: 赛题原始需求目标≥40%，非声称结果，权威实测为-14.0% --> |
...
| R-F-04 | "量子退火将策略网络权重映射为 QUBO 问题（当前经典模拟退火），20seed 权威方向为 -5.6%（p=0.9430不显著，训练开销+74.5%，探索性方向；旧 +6.4% 已废弃）" | 第 7 页 | ✅ 已同步 |
| R-F-05 | "MAPPO 多智能体协同调度，3 台真机负载均衡 CV=0.246，同训练量收敛严格对比下协同优势 +36.5%（MAPPO vs 独立PPO，N=20，p=0.024；旧 +84.6% 为未收敛+训练量不均等的探索值、旧 +4.0% 为 N=10 口径，均已废弃）" | 第 7 页 | ✅ 已同步 |
| R-P-01 | "PPO 综合奖励提升 20.2%（N=250）；量子比特利用率权威口径为 -3.3%（PPO 0.4467 vs FCFS 0.4620，N=250），≥30% 目标未达成，不作为价值主张" | 第 9 页 | ✅ 已同步 |
| R-P-05 | "PPO 在 4 种压力场景下综合稳定性最强，量子波动场景比第二名提升 91.4%（历史探索数据，诚实化前旧 FCFS 基线，权威 stress 数据待重跑核定）" | 第 12 页 | ✅ 已同步 |
...
| R-F-05 | "MAPPO 多智能体协同调度，同训练量收敛严格对比下协同优势 +36.5%（N=20，p=0.024；旧 +84.6% 为未收敛探索值、旧 +4.0% 为 N=10 口径，均已废弃）" | 段落 3：核心技术展示 | ✅ 已同步 |
| R-P-05 | "量子波动场景比第二名提升 91.4%（历史探索数据，诚实化前旧基线，权威 stress 数据待重跑核定）" | 段落 5：性能对比 | ✅ 已同步 |
...
| 风险点 | 可能追问 | 应对策略 |
|-------|---------|---------|
| 等待时间指标未达 40% | "为什么等待时间没有降低？" | 权威实测（N=250）PPO 平均等待时间 25.46 步，低于 FCFS 29.61 步（**-14.0%，PPO 更优**），即吞吐量与等待时间双优。未达赛题 ≥40% 目标，但优于基线且无负权衡。答辩口径：「综合调度收益提升 20.2%（N=250, p<0.001），平均等待时间亦优于 FCFS（-14.0%），双指标优于基线」 |
| head_only 退火有效性 | "head_only 退火真的有效吗？" | 引用 Issue #104 的验证报告（待完成） |
| 奖励函数消融缺失 | "奖励函数各组件的贡献度？" | D3 消融已完成（`results/reports/reward_ablation_d3.md`）：6 配置 × 2 策略 × 25 episodes，证明执行奖励权重贡献 -834，等待惩罚贡献 -1078，各组件有明确语义贡献 |
