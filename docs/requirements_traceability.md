...
| R-E-02 | 测试覆盖率 ≥80% | 工程 | `tests/` | **已达成** | pytest-cov | "测试覆盖率 93.58%，3697 测试用例，94+ 个测试文件" | `tests/` 目录 |
| R-E-02 | 测试覆盖率 ≥80% | 工程 | `tests/` | **已达成** | pytest-cov | "测试覆盖率 93.58%，3697 测试用例，94+ 个测试文件" | `tests/` 目录 |
...
|---------|---------|---------|---------------------|-------|------|
| R-P-02 | 吞吐量+20.2%（等待时间为帕累托权衡） | **权衡边界**：PPO 等待时间高于 FCFS，D3 消融证明无法通过推理时调整 reward weight 改变 | **否** | 中 | D3 消融实验（6配置×2策略×25ep）证明：推理时调整 reward weight 不改变已训练 PPO 的 policy；等待时间是多目标优化的帕累托权衡维度，核心优势为吞吐量+20.2%。详见 `results/reports/reward_ablation_d3.md` |
...
| R-P-01 | "PPO 综合奖励提升 20.2%（N=250）；利用率数据来自10seed对比（PPO 42.5% vs FCFS 43.0%，需重算，Issue #116）" | 第 9 页 | ✅ 已同步（Issue #116 修复） |
...
| 风险点 | 可能追问 | 应对策略 |
|-------|---------|---------|
| 等待时间指标未达 40% | "为什么等待时间没有降低？" | D3 消融实验证明：推理时调整 reward weight 不改变已训练 PPO 的 policy（等待时间恒定 57.73 步）；PPO 训练目标是综合奖励最大化，等待时间是资源利用率的权衡维度。答辩口径：「综合调度收益提升 20.2%（N=250, p<0.001），等待时间是当前多目标优化的权衡边界」 |
