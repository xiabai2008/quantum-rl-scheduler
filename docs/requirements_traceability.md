...
| R-F-05 | 支持多机器协同调度 | 功能 | `src/scheduler/marl.py`<br>`src/scheduler/env.py` | **已实现** | 同训练量收敛严格对比 | "MAPPO 多智能体协同调度，3 台真机负载均衡 CV=0.246，同训练量收敛严格对比下协同优势 +4.0%（MAPPO vs 独立PPO；旧 +84.6% p=0.0188 为未收敛+训练量不均等的探索值，已废弃）；叠加规模扩展效应后总提升 vs 单机 +86.3%" | `results/reports/mappo_strict_comparison_report.md` |
...
| R-E-02 | 测试覆盖率 ≥80% | 工程 | `tests/` | **已达成** | pytest-cov | "测试覆盖率 ≥80%（CI 门禁 --cov-fail-under=80），主套件 3723 测试用例，94+ 个测试文件" | `tests/` 目录 |
...
| R-F-05 | "MAPPO 多智能体协同调度，3 台真机负载均衡 CV=0.246，同训练量收敛严格对比下协同优势 +4.0%（MAPPO vs 独立PPO；旧 +84.6% p=0.0188 为未收敛+训练量不均等的探索值，已废弃）" | 第 7 页 | ✅ 已同步 |
...
| R-F-05 | "MAPPO 多智能体协同调度，同训练量收敛严格对比下协同优势 +4.0%（旧 +84.6% 为未收敛探索值，已废弃）" | 段落 3：核心技术展示 | ✅ 已同步 |
