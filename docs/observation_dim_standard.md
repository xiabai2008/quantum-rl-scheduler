...
> **Issue #129 / #404 / #830** | 最后更新: 2026-07-31
...
| **17维（可选·公平性扩展）** | 16维基础上新增Jain公平性指数观测 | `include_fairness_obs=True`（默认） | `Box(0, 1, (17,))` | ✅ v9.1新增（Issue #654/#830） |
...
| 16 | OBS_JAIN_FAIRNESS | Jain公平性指数（0-1归一化） | 公平性（v9.1新增） |

### 1.3 17维观测空间（可选·公平性扩展）

**定义文件**：`src/scheduler/env.py`（`include_fairness_obs` 参数，Issue #654）

- **索引16**：Jain公平性指数（0-1归一化），衡量多用户调度的公平性
- **开关**：`include_fairness_obs` 参数（默认 `True`，Issue #654）
- **向后兼容**：设为 `False` 时回退到16维标准观测空间
- **CHANGELOG**：v9.1 新增第17维可选公平性指数观测（`include_fairness_obs`开关控制Jain公平性指数）
...
| 编译PPO vs SABRE | 深电路(14-16q, 20电路)SWAP减少+45.5%(方向性优势，样本量不足) | 编译层14维, 4×4 2D网格 | 公平对比v2, 60电路同池配对, 全60 p=5.95e-01不显著, 中+深40 p=2.56e-01不显著 |
