...
| 答辩口径 | +20.2% 基于 16 维交付模型 N=250 实验（config/statistics.yaml，v9.1+已验证）；14维旧模型（已删除）历史参考 |
...
| 指标 | 值 | 维度/环境 | 来源 |
|:--|:--|:--|:--|
| 仿真 PPO 均值 | 1982.69 ± 557.25 | **16维交付模型（新奖励参数）** | 50 seeds × 5 episodes = N=250 |
| 仿真 FCFS 均值 | 1648.91 ± 502.95 | **16维交付模型（新奖励参数）** | 同上 |
| 仿真 PPO 提升 | +20.2% | **16维交付模型（新奖励参数）** | 同上（Welch t, p=7.56e-12） |
| 编译PPO vs SABRE | 深电路N=200规模化扩充（8.14, Issue #559）：SWAP减少+52.1%（Wilcoxon p=2.30e-10高度显著，d_z=0.54） | 编译层14维, 4×4 2D网格 | 公平对比v2, 60电路同池配对, 全60 p=8.40e-01不显著, 浅/中电路无优势 |
| 2D网格拓扑优势 | SWAP减少~62% vs 线性链 | 编译环境 | 拓扑消融（BFS验证） |
| 交付模型 | `ppo_best_model_16dim.zip` | 16维调度 | PPO-MLP, 100K steps |

> **重要声明（Issue #559/#530，v9.1+更新）**：+20.2%权威数字基于**16维交付模型 + 新奖励参数**的50seed实验（N=250, config/statistics.yaml），
> 对应模型为 `ppo_best_model_16dim.zip`。
> 14维旧模型（`ppo_best_model_14dim.zip`，已删除）使用旧奖励参数，已被v9.1+ 16维交付模型替代。
> v9.1+已完成N=250多seed评估验证，+20.2%为16维交付模型权威对比结果。
