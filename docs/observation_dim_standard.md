...
> **重要声明（Issue #559/#530）**：+88.3%权威数字基于**14维调度模型 + 旧奖励参数**的50seed实验（N=250, config/statistics.yaml），
> 对应模型为已删除的 `ppo_best_model_14dim.zip`。
> 16维交付模型（`ppo_best_model_16dim.zip`）使用新奖励参数（`REWARD_CLASSICAL` 3.0→5.0 等），
> 修复了观测维度不匹配的P0问题，确保评估脚本可一键运行，但**尚未以相同protocol（50seeds×5episodes=N=250）跑多seed评估**。
> 16维模型在新奖励参数下的性能是否维持 +88.3% 量级完全未知，需重新验证后才能更新权威统计口径。
