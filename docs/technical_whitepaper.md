...
在50 seeds × 5 episodes（N=250）的严格统计评估中，PPO相比FCFS基线实现了**+88.3%的综合调度性能提升**（Mann-Whitney U检验，p=1.032×10⁻⁴²，rank-biserial效应量=-0.71）。系统已完成与天衍量子计算云平台的SDK全链路对接，**315次调用成功率100%**。本文同时诚实报告了方法的局限性：等待时间与吞吐量之间存在帕累托权衡，量子赋能AI方向（量子退火加速PPO决策头，p=0.9430）的性能提升未达统计显著水平。
...
|:--:|:--|:--:|:--:|:--:|:--|
| **1** | **PPO** | **2746.94** | **1160.72** | **+88.3%** | **1.032e-42** |
...
- 统计显著性极强：p=1.032e-42，效应量rank-biserial=-0.71（大效应量）
...
### 4.7 真机噪声反馈鲁棒性探索 (Exploratory Study with Real-Machine Noise Feedback)

> ⚠️ **统计局限性声明（Issue #528 / #529 / #532）**：本节为**探索性研究**，不构成统计成立的证据链。
> - 10seeds 分布实验未做 Wilcoxon 配对检验 / 效应量 / 95% CI，无法判定 +6.1% 是否统计显著；
> - 单seed 校准显示等待时间 -5.7%（改善），10seeds 分布实验显示 +6.1%（恶化），**方向相反**；
> - 早期版本中的 "p=0.04（显著）" 声明在全仓库任何实验报告 / JSON 中均无出处，已删除（Issue #528）；
> - 详见 `results/reports/quantum_noise_10seeds_20260727_142756.md` 与 `quantum_noise_calibration_20260727_141633.md`。

为了探索“量子赋能AI”闭环，我们设计了实验，将从天衍-287真机提取的噪声模型注入仿真环境，对比噪声校准前后PPO策略的性能变化。

**探索性发现：以少量延迟为代价，可能换取策略鲁棒性（待统计验证）**

| 对比策略 | 平均等待时间变化 (10 seeds, 描述性) | 统计检验 |
|:---|:---:|:---:|
| PPO (噪声校准) vs PPO (标准) | **+6.1%**（恶化） | ⚠️ 未做（探索性） |

| 单seed 校准 (历史对照) | 等待时间变化 | 方向 |
|:---|:---:|:---:|
| PPO (噪声校准) vs PPO (标准) | -5.7%（改善） | 与 10seeds 相反 |

- **结果解读**: 10seeds 描述性统计显示，注入真实噪声后，PPO 智能体的平均等待时间**增加**了 6.1%；而单seed 校准则显示等待时间**减少** 5.7%。两套数据方向相反，仅可作为探索性观察。
- **工程权衡假设（待验证）**: 一种可能的解释是智能体在感知到真实硬件噪声后，学习到了更保守、更可靠的调度策略（例如避免在噪声高的比特上执行关键任务），其代价是牺牲了一部分等待时间。但该假设目前**缺乏统计支撑**，需补做 Wilcoxon 配对检验 + 效应量 + 鲁棒性量化指标（任务失败率 / 保真度）后才能下结论。
- **结论**: 真机噪声反馈方向目前为**探索性研究**，不作为核心 claim。"以少量延迟换取更高任务成功率和结果可靠性"的叙事需在补做统计检验后再作判断，详见 `docs/bidirectional_empowerment.md` 4.3 节统计局限性声明。
...
1. **提升综合调度收益**：通过智能调度，综合调度奖励提升 ≥30%（PPO 实测 +88.3%, N=250, p=1.032e-42）；量子比特利用率为多目标权衡维度（Issue #440），对应机时成本节省
