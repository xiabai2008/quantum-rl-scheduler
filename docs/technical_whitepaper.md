...
量子计算正从实验室走向产业化，但异构量子-经典混合计算环境下的任务调度仍是关键瓶颈。现有量子云平台普遍采用先来先服务（FCFS）等简单启发式规则，无法有效应对动态任务到达、异构算力分配和服务等级目标（SLO）约束。本文提出一种基于近端策略优化（PPO）的强化学习调度系统，通过**16维观测空间**感知量子比特状态、队列特征、物理噪声、拓扑信息、串扰风险及时序流量（到达率MA），在量子、经典、混合以及**动态误差缓释（QEM）**四种执行模式间做出智能分配决策。

在最新版本中，系统实现了三大前沿突破：**1. 动态量子误差缓释（Dynamic QEM）**，在执行时间与保真度之间智能权衡；**2. 串扰感知空间并发（Crosstalk-Aware Spatial Scheduling）**，在多任务并发时主动规避拓扑串扰；**3. 时序流量感知（Sequence-Aware Scheduling）**，通过观测空间中的到达率滑动平均感知流量趋势。（注：RecurrentPPO/LSTM为消融实验对比项，交付模型使用标准PPO-MLP，见§3.2）
...
采用MLP（多层感知机）策略网络架构（交付模型 `ppo_best_model_16dim.zip`，SB3 `MlpPolicy`，`net_arch=[64, 64]`）：
- 输入层：16维状态向量
- 隐藏层1：64神经元，ReLU激活
- 隐藏层2：64神经元，ReLU激活
...
> **注**：RecurrentPPO（LSTM）作为消融实验对比项保留，消融实验验证MLP与LSTM收敛到相同策略，MLP训练效率更高且兼容性更好（见 `docs/observation_dim_standard.md` §3.1）。未来工作可探索RecurrentPPO时序建模。
...
3. **时序流量感知 (Sequence-Aware Scheduling)**
   - 机制：将环境的“任务到达率滑动平均”编码进观测空间（第15维 OBS_ARRIVAL_RATE_MA），使标准PPO-MLP策略也能感知流量趋势。
   - 效果：Agent 不再是被动地“看到任务才分配”，而是能感知到“流量正在激增”或“处于低谷”。在即将到来的潮汐洪峰前，Agent 能学会预留高质量的量子资源，避免因短视导致的资源耗尽。
   - **注**：RecurrentPPO（LSTM）为消融实验对比项，消融验证MLP与LSTM收敛到相同策略；未来工作可探索LSTM隐式时序建模。
