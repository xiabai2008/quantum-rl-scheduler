...
量子计算正从实验室走向产业化，但异构量子-经典混合计算环境下的任务调度仍是关键瓶颈。现有量子云平台普遍采用先来先服务（FCFS）等简单启发式规则，无法有效应对动态任务到达、异构算力分配和服务等级目标（SLO）约束。本文提出一种基于近端策略优化（PPO）的强化学习调度系统，通过**16维观测空间**感知量子比特状态、队列特征、物理噪声、拓扑信息、串扰风险及时序流量，在量子、经典、混合以及**动态误差缓释（QEM）**四种执行模式间做出智能分配决策。

在最新版本中，系统实现了三大前沿突破：**1. 动态量子误差缓释（Dynamic QEM）**，在执行时间与保真度之间智能权衡；**2. 串扰感知空间并发（Crosstalk-Aware Spatial Scheduling）**，在多任务并发时主动规避拓扑串扰；**3. 时序流量感知（Sequence-Aware Scheduling）**，通过任务到达率滑动平均观测维度感知流量趋势。
...
选择Proximal Policy Optimization（PPO）作为核心算法，交付模型采用标准PPO-MLP架构（`use_lstm=False`）。消融实验验证MLP与LSTM收敛到相同策略，MLP训练效率更高且兼容性更好（标准`PPO.load()`可直接加载，无需RecurrentPPO依赖）。

| 对比维度 | PPO-MLP（交付） | RecurrentPPO-LSTM（消融） | DQN |
|:--|:--|:--|:--|
| 离散/连续动作 | 两者皆可 | 两者皆可 | 仅离散 |
| 时序感知能力 | 通过到达率MA观测维度 | 强（隐藏状态记忆） | 无 |
| 训练稳定性 | 高（裁剪目标函数） | 中高 | 中（经验回放） |
| 样本效率 | 中 | 中 | 高 |
| 模型兼容性 | ✅ 标准PPO.load() | ⚠️ 需RecurrentPPO | ✅ 标准 |

PPO通过裁剪的代理目标函数（clip range=0.2）限制每次策略更新的幅度。消融实验证实，在本调度任务上MLP与LSTM收敛到同一最优策略，因此交付模型选用更简洁高效的MLP架构。时序流量感知通过观测空间中的"任务到达率滑动平均"维度实现，无需LSTM隐状态。

### 3.2 网络架构

采用标准PPO-MLP网络架构（交付模型 `ppo_best_model_16dim.zip`）：
- 输入层：16维状态向量
- 隐藏层1：64神经元（ReLU激活）
- 隐藏层2：64神经元（ReLU激活）
...
> **消融实验结论**：MLP与LSTM架构收敛到相同最优策略。MLP训练更快、推理无需维护LSTM隐状态、兼容标准`PPO.load()`，因此选为交付模型。LSTM仅用于消融实验参考（`scripts/evaluation/ablation_ppo_variants.py`）。
...
3. **时序流量感知 (Sequence-Aware Scheduling)**
   - 机制：将环境的"任务到达率滑动平均"编码进观测空间第15维（OBS_ARRIVAL_RATE_MA），使PPO-MLP策略能感知流量趋势。
...
| 真机性能对比 PPO vs FCFS | +353.4%（10seeds v2，⚠️小样本探索性结果，混合环境非纯真机对比） |
