...
在50 seeds × 5 episodes（N=250）的严格统计评估中，PPO相比FCFS基线实现了**+88.3%的综合调度性能提升**（Mann-Whitney U检验，p=1.032×10⁻⁴²，rank-biserial效应量=-0.71）。系统已完成与天衍量子计算云平台的SDK全链路对接，**284次真机调用成功率100%**。本文同时诚实报告了方法的局限性：等待时间与吞吐量之间存在帕累托权衡，量子赋能AI方向（量子退火加速DQN）的性能提升未达统计显著水平。
