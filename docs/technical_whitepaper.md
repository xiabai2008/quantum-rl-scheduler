...
在50 seeds × 5 episodes（N=250）的严格统计评估中，PPO相比FCFS基线实现了**+123.4%的综合调度性能提升**（Welch t检验，p=1.449×10⁻⁶⁶，Cohen's d效应量=-2.1353）。系统已完成与天衍量子计算云平台的SDK全链路对接，**315次调用成功率100%**。本文同时诚实报告了方法的局限性：等待时间与吞吐量之间存在帕累托权衡，量子赋能AI方向（量子退火加速PPO决策头，p=0.9430）的性能提升未达统计显著水平。
...
1. **提升综合调度收益**：通过智能调度，综合调度奖励提升 ≥30%（PPO 实测 +123.4%, N=250, p=1.449e-66）；量子比特利用率为多目标权衡维度（Issue #440），对应机时成本节省
