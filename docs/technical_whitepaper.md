...
> ✅ **统计证据声明（v9.1+，N=25 配对检验）**：本节主结论已通过配对统计检验，统计成立。
...
>
> ⚠️ **解读边界**：该证据证明 PPO 策略能感知量子硬件真实噪声波动，“真机测量→训练环境→策略”闭环成立；不应表述为“量子提升 AI 性能”。
...
- **结果解读**：N=25 配对检验证明真机噪声分布对 PPO 策略奖励有显著影响，即 PPO 能感知量子硬件真实噪声波动，量子硬件测量结果反哺 AI 训练环境形成闭环，符合赛题「量子计算赋能 AI」中的硬件噪声感知训练方向。
...
| DQN 占位 vs FCFS | -160.06 | — | — | 小 | ⚠️ DQN 已删除（Random 占位） |
...
### 11.2 结果（2026-07-31 重测）

| 子集 | PPO vs SABRE | p 值 | 结论 |
|:--|:--|:--|:--|
| 全 60 电路 | +16.5%（SWAP 8.12 vs 9.72） | 8.40e-01 | ❌ 不显著 |
| 中+深 40 | +19.7% | 5.99e-01 | ❌ 不显著 |
| 深电路 20 | +33.3% | —（n=20 方向性） | ⚠️ 方向性优势 |

**诚实声明**：编译层 PPO 相对 SABRE 的差异未达统计显著性，本项目将其定位为"探索性验证 + 工程管线"，不作为核心性能声明；调度层 +123.4% 的显著提升才是主证据。
...
- **N=25 配对检验**：25 seeds × 5 episodes × 2 条件（无噪声 vs 真机噪声分布），同一 seed 配对
...
真机 N=10 历史实验（d=5.33）为小样本探索性结果；N=25 配对检验为当前权威口径（`docs/real_machine_verification_boundary.md`）。
