...
1. **等待时间权衡**：PPO以+51%的等待时间为代价换取+20.2%的吞吐量，等待时间是权衡而非优化。不同SLO需求可通过奖励权重调整。
...
**诚实声明**：编译层 PPO 在全 60 电路（含浅/中电路）上差异不显著（p=8.40e-01），说明对简单电路 SABRE 已足够；**在深电路（14-16 qubits 大规模电路）上显著优于 SABRE（p<0.05，规模化效应 p<1e-7）**——编译层定位为"探索性验证 + 深电路显著子集"，调度层 +20.2% 的显著提升仍是主证据。
