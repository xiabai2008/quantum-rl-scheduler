...
1. **量子比特利用率未达成目标**：权威口径下 PPO 量子比特利用率 -3.3%（PPO 0.4467 vs FCFS 0.4620，N=250），发榜方技术文档/团队需求基线 R-P-01 的 30% 目标未达成（注：30% 目标来自团队需求追溯矩阵，非比赛方案明文要求），利用率不作为价值主张。等待时间维度上 PPO 平均等待时间更短（-14.0%，25.46 vs 29.61 步），无"以等待时间换吞吐量"的负权衡。
...
2. **价值主张以"场景-能力"映射为准**：本章不进行金额估算（ROI 见 §6.2），量子比特利用率权威口径 -3.3%（PPO 0.4467 vs FCFS 0.4620，N=250）未达赛题 ≥30% 目标，不作为价值主张
