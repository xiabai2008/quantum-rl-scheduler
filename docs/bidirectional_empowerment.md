...
> **定位声明**：本系统以"AI赋能量子计算智能调度"为核心方向，在仿真环境中取得统计显著的调度性能提升。"量子赋能AI"为探索性研究方向，当前量子启发式退火模块使用经典模拟退火（D-Wave neal），训练时间增加74.5%，最终奖励提升+6.4%在5 seeds下统计不显著（p=0.190），作为未来工作方向而非核心claim。
...
- 8 策略排名：PPO(2746.94) > SJF(1462) > DQN(1510, +3.0% vs FCFS, 公平重训14维) > FCFS(1459) > Random(1247) > Greedy(-26) > Quantum-Only(-920.54) > Classical-Only(-1128.29)
...
| 指标 | 退火开启 | 退火关闭 | 提升 |
|------|---------|---------|------|
| PPO 训练时间 | baseline | — | **开销+74.5%（探索性）** |
| 最终策略奖励提升 | baseline | — | **+6.4%（p=0.190, 不显著）** |
...
**方案**：将调度决策变量编码为 QUBO 二元变量矩阵，映射到量子比特哈密顿量，通过量子启发式退火（D-Wave 同款算法，当前为经典模拟退火）在多项式时间内逼近全局最优解。

**证据**：

- 已支持的 QUBO 规模：head_only 模式 260 变量；分层全网络扩展中架构已就绪
- 量子求解器：本地 D-Wave neal + 天衍云超导量子平台（tianyan176，66 qubit，可用性验证）
...
```
量子启发式退火优化参数 ──► PPO 策略更新 ──► 更优调度决策 ──► 更高量子利用率 ──►
    ▲                                                                      │
    └──────────────────── 更多真机反馈数据 ──────────────────────────────┘
```
...
| 量子→AI | 退火训练时间 | **开销+74.5%（不显著）** |
| 量子→AI | 最终奖励提升 | **+6.4%（p=0.190, 不显著）** |
