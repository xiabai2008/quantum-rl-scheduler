...
**问题**: ~~写的是 "真机调用成功率 100%（284 次）| 天衍-176 真机验证"~~（已修正为315次口径统一）
但最新数据是 **天衍-287 多seed真机实验**（2026-07-24）：
- PPO=1736.32±355.78 vs FCFS=383.00±49.13
- Cohen's d=5.33（效应量异常大，小样本探索性结果需验证）, Welch p<0.001（Bonferroni校正后显著）
...
- 多seed真机：PPO=1736.32±355.78 vs FCFS=383.00±49.13, Cohen's d=5.33（效应量异常大，小样本探索性结果需验证）, p<0.001（Bonferroni校正后显著）
- 所有实验数据和PR必须与以上数字一致
