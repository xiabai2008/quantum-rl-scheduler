...
| **本系统 (v9.1)** | **2026** | **本项目** | **PPO-MLP + QEM** | **多目标（吞吐/等待/利用率/串扰）** | **287（天衍-287）** | **+20.2%，p=7.56e-12** | **据调研首个集成误差缓释、串扰感知与多目标公平调度的RL量子调度系统** |
...
3. **统计严谨性（学术标准）**：50 seeds × 5 episodes = N=250，Welch t检验 p=7.56e-12，Cohen's d=-2.1353（权威源: config/statistics.yaml），现有方法多数无统计检验
...
- **本系统定位**：N=250，Welch t 检验 p=7.56e-12，Cohen's d=-2.1353，Bonferroni 校正——**统计严谨性据调研在同类工作中处于领先**
...
> **诚实披露**：项目主结果 +20.2% 使用了**弱基线**（固定返回混合动作的 `FCFSStrategy`），与更智能的**观测感知基线**（`EnvBasedFCFSScheduler`）对比时优势缩水至 +11.4% 且不显著。为避免评委误以为 +20.2% 是相对于"工业级 FCFS"的提升，特此双基线对照。

| 基线类型 | FCFS 实现 | PPO 提升 | p 值 | N | 显著性 | 实验脚本 |
|:--|:--|:--:|:--:|:--:|:--:|:--|
| 弱基线（权威主结果） | `FCFSStrategy`（固定返回混合动作） | **+20.2%** | 7.56e-12 | 250 | 显著（Bonferroni 校正后） | `run_multiseed_evaluation.py` |
| 观测感知基线（智能 FCFS） | `EnvBasedFCFSScheduler`（根据任务类型和量子可用性选择动作） | **+11.4%** | 0.2235 | 50 | **不显著**（Cohen's d=0.2450，小效应量） | `sota_comparison.py` |

**结论**：
- 主结果 +20.2% 的统计显著性极强，但建立在弱基线之上；对观测感知 FCFS 仅 +11.4% 不显著（N=50）。
- 两个实验的结论方向一致（PPO 优于 FCFS），但优势幅度受 FCFS 实现复杂度和样本量影响。
- **答辩话术建议**：诚实披露双基线口径，强调 +20.2% 是相对于"无智能的固定动作 FCFS"，对"观测感知 FCFS"的优势需扩大 N 至 ≥100 才能获得结论性 p 值。
...
|:----:|:--------:|:------:|:------------:|:----:|
| **PPO** | **1982.69** | 857.25 | **+20.2%** | 1 |
| SJF | 1060.30 | 109.71 | +0.8% | 2 |
| FCFS | 1648.91 | 58.34 | 基线 | 3 |
...
|:----:|:--------:|:----:|:-----------------------:|:-----------------:|
| PPO vs FCFS | Welch t | 7.56e-12 | Cohen's d=-2.1353（大效应） | 显著 |
