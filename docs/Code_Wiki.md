...
- **AI 赋能量子计算**：用强化学习（RL）智能调度量子/经典混合任务，量化目标为综合调度收益 +123.4%（核心目标，已达成，N=250, p=1.449e-66）；资源利用率 -3.3%（N=250 真实 FCFS 基线下，8.5 审查诚实化）
...
| v9.1 | **16维交付模型 + 公平性特性 + 编译环境可配置 + N=250统计验证** | 标准PPO-MLP交付模型（ppo_best_model_16dim.zip），公平性惩罚机制(#587)+公平性观测开关(#588)+观测维度消融配置(#585)，编译环境支持自定义拓扑(#594/#616)，50seed N=250 Welch t检验 p=1.449e-66，Cohen's d=-2.1353，覆盖率93.58%，3523测试用例 |
