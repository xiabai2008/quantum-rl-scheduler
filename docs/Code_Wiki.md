...
| v9.1 | **16维交付模型 + 公平性特性 + 编译环境可配置 + N=250统计验证** | 标准PPO-MLP交付模型（ppo_best_model_16dim.zip），公平性惩罚机制(#587)+公平性观测开关(#588)+观测维度消融配置(#585)，编译环境支持自定义拓扑(#594/#616)，50seed N=250 vs 真实 FCFS：Welch t p=7.56e-12（8.5 基线诚实化，旧 p=1.449e-66 对应 Hybrid-Default 基线），CI 覆盖率≥80%（强制），3720 测试用例 |
...
测试目录共 **94 个测试文件**，**3720 个测试用例**（主套件口径，pytest --collect-only 实测，2026-08-08；全量 3741 含 benchmark），CI 覆盖率强制 ≥80%。
