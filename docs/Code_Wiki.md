...
> **目标平台**：天衍云真机"天衍-287"（105数据比特+182耦合比特超导量子计算机，搭载祖冲之三号同款芯片）
> **文档版本**：v9.1（16维交付模型+N=250统计验证+真机闭环+编译层公平对比v2+噪声反馈配对检验）
> **最后更新**：2026-07-31
>
> **⚠️ 注意**：本文档中引用的源码行号（如 `L59-756`）为编写时的近似值，随代码重构可能已偏移，请以函数/类名为准检索源码。
...
- **AI 赋能量子计算**：用强化学习（RL）智能调度量子/经典混合任务，量化目标为综合调度收益 +20.2%（核心目标，已达成，N=250, p=7.56e-12）；资源利用率 -3.3%（N=250 真实 FCFS 基线下，8.5 审查诚实化）
- **量子赋能 AI**：主方向——真机噪声特征建模与PPO鲁棒性评估（天衍-287 H门1024 shots→保真度0.976→噪声分布建模→PPO噪声鲁棒性基准）；退火模块为探索性功能，默认关闭（20seeds统计不显著 p=0.9430，实为经典模拟退火）

**技术栈**：Python ≥3.10 + Stable-Baselines3（DQN/PPO）+ Gymnasium + PyTorch + Qiskit + D-Wave Ocean SDK + FastAPI + Vue3 + Echarts

**核心代码量**：约 1.2 万行 Python（src/ 68 文件，不含测试和文档），94 个测试文件，3523 个单元测试用例（pytest --collect-only 实测，2026-07-31）。
...
| v8 | 50seed N=250验证 + 多seed真机实验 | PPO=1982.69±557.25, +20.2%, p=7.56e-12, rank-biserial=-0.3642；多seed真机PPO=1632.26±326.49 vs FCFS=782.94±467.61, d=2.11（效应量异常大，小样本探索性结果，需进一步验证）| | FCFS: 1648.91±502.95
| v9 | **Dynamic QEM + Crosstalk-Aware + Sequence-Aware (LSTM)** | 状态空间扩充至 **16维**（新增串扰风险、任务到达率MA），动作空间扩充至 **4维**（新增量子误差缓释动作）。实现空间并发与 LSTM 时序流量感知。 |
| v9.1 | **16维交付模型 + 公平性特性 + 编译环境可配置 + N=250统计验证** | 标准PPO-MLP交付模型（ppo_best_model_16dim.zip），公平性惩罚机制(#587)+公平性观测开关(#588)+观测维度消融配置(#585)，编译环境支持自定义拓扑(#594/#616)，50seed N=250 vs 真实 FCFS：Welch t p=7.56e-12（8.5 基线诚实化，旧 p=1.449e-66 对应 Hybrid-Default 基线），CI 覆盖率≥80%（强制），3725 测试用例 |
...
> **可选第 17 维**：`include_fairness_obs=True` 时追加 Jain 公平性指数作为第 17 维观测（默认关闭，以保持 16 维交付模型 `ppo_best_model_16dim.zip` 的观测空间兼容性，详见 `docs/observation_dim_standard.md`）。
...
| PPO + LSTM | `PPOAgent(use_lstm=True)` | 消融实验用，时序流量预测；交付模型为标准PPO-MLP（MLP=LSTM性能等价但训练快5倍） |
...
| `ANNEALING_ENABLED` | 退火启用 | false（默认关闭，探索性功能，p=0.9430不显著，@deprecated） |
...
测试目录共 **94 个测试文件**，**3725 个测试用例**（主套件口径，pytest --collect-only 实测，2026-08-08；全量 3746 含 benchmark），CI 覆盖率强制 ≥80%。
...
# 排名：PPO(1982.69) > FCFS(1648.91) > SJF(748.48) > DQN/Random(697.40)
#       > Greedy(62.72) > Quantum-Only(-826.59) > Classical-Only(-1075.49)
```
...
项目提供多阶段 Docker 构建（[Dockerfile](../Dockerfile)）：
