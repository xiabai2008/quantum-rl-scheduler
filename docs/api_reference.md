...
> **权威实验数字**（文档引用须与此一致）：50seed 仿真 PPO=1982.69±557.25 vs FCFS=1648.91±502.95，提升 +20.2%，Welch t 检验，p=7.56e-12，rank-biserial=-0.3642；多 seed 真机 PPO=1632.26±326.49 vs FCFS=782.94±467.61，d=2.11，p<0.001（Bonferroni 校正后显著）。
...
|---------|------|-----------|------|
| `tianyan_s` | 天衍-S | 287（105+182） | 超导量子计算机（仿真占位） |
| `tianyan_sw` | 天衍-SW | 287（105+182） | 超导量子计算机（仿真占位） |
| `tianyan_tn` | 天衍-TN | 287（105+182） | 超导量子计算机（仿真占位） |
| `tianyan_annealer` | 天衍退火器 | - | 退火求解器（仿真模拟退火） |

> 注：上表为系统内置仿真占位机器。接入的真实天衍云机器为：天衍-287（287 总比特：105 数据 + 182 耦合）、tianyan176（176 比特）、tianyan-294（294 比特）。
