...
def build_qubo_matrix(
    ...
    8.5 审查注明：当前构造使所有矩阵元素非负（仅任务选择线性项 + 成对惩罚项），
    数学最优解为"什么都不选"（全 0 决策）——该矩阵仅用于退火权重 L2 范数
    实验（非策略性能目标），不在实时调度循环中使用；若用于任务选择需引入
    负的奖励系数项（当前未实现，为已知局限）。
    """
    """
...
if __name__ == "__main__":
    ...
    class MockAgent:
        ...
        def __init__(self, state_dim=8, action_dim=3):
            # action_dim=3 是独立 mock 值，与 env_types.N_ACTIONS 无关（Issue #787）
