...
class LearnableMachineScorer(nn.Module):
    """可学习机器评分器（Issue #598）。

    替代静态评分公式 ``fidelity * available_ratio / (1 + quantum_queue)``，
    使用小型 MLP 学习机器特征到评分的映射，与 Actor-Critic 联合优化。

    输入特征（3维，归一化到 [0, 1]）：
    - fidelity        : 量子比特保真度
    - available_ratio : 可用比率
    - quantum_queue   : 量子队列长度（归一化）

    网络结构：3 → hidden → hidden → 1（输出标量评分）

    Attributes:
        input_dim   : 输入特征维度（固定为 3）
        hidden_dims : 隐藏层尺寸
    """

    # 机器特征维度：fidelity, available_ratio, quantum_queue
    MACHINE_FEATURE_DIM: int = 3

    def __init__(self, hidden_dims: tuple[int, ...] = (32, 16)) -> None:
        """初始化可学习评分器。

        Args:
            hidden_dims: 隐藏层尺寸，默认 (32, 16)
        """
        super().__init__()
        self.input_dim: int = self.MACHINE_FEATURE_DIM
        self.hidden_dims: tuple[int, ...] = hidden_dims
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            features: 形状 (batch_size, 3) 或 (3,) 的张量

        Returns:
            形状 (batch_size, 1) 或 (1,) 的评分张量
        """
        return cast(torch.Tensor, self.net(features))

    def score_machine(self, fidelity: float, available_ratio: float, quantum_queue: float) -> float:
        """计算单台机器的评分（便捷方法，用于推理时替代 _machine_score）。

        Args:
            fidelity        : 量子比特保真度（0-1）
            available_ratio : 可用比率（0-1）
            quantum_queue   : 归一化量子队列长度（0-1）

        Returns:
            评分值（越高越优）
        """
        features = torch.tensor(
            [fidelity, available_ratio, quantum_queue],
            dtype=torch.float32,
            device=next(self.parameters()).device,
        )
        with torch.no_grad():
            return float(self(features.unsqueeze(0)).item())

    @staticmethod
    def extract_features(
        fidelity: float, available_ratio: float, quantum_queue: float
    ) -> np.ndarray:
        """提取机器特征向量（静态方法，供外部调用）。
        ...
        return np.array(
            [
                ...
                float(np.clip(available_ratio, 0.0, 1.0)),
                float(np.clip(quantum_queue, 0.0, 1.0)),
            ],
            dtype=np.float32,
        )
...
class MultiAgentEnvWrapper:
    ...
    def __init__(self, env: QuantumSchedulingEnv, machine_scorer: LearnableMachineScorer | None = None):
        ...
        # Issue #598: 可学习评分器
        self.machine_scorer: LearnableMachineScorer | None = machine_scorer
    ...
    def _machine_score(self, machine_idx: int) -> float:
        ...
        if self.machine_scorer is not None:
            # Issue #598: 使用可学习评分器
            quantum_queue_norm = float(np.clip(m.quantum_queue / MAX_QUEUE_SIZE, 0.0, 1.0))
            return self.machine_scorer.score_machine(
                fidelity=float(m.fidelity),
                available_ratio=float(m.available_ratio),
                quantum_queue=quantum_queue_norm,
            )
...
class MultiAgentPPO:
    ...
    def __init__(
        ...
        verbose: int = 1,
        learnable_scorer: bool = False,
        scorer_hidden: tuple[int, ...] = (32, 16),
        ...
        # Issue #598: 可学习路由评分器
        self.machine_scorer: LearnableMachineScorer | None = None
        if learnable_scorer:
            self.machine_scorer = LearnableMachineScorer(scorer_hidden)
        self.wrapper = MultiAgentEnvWrapper(env, machine_scorer=self.machine_scorer)
        ...
        # Issue #598: 可学习评分器优化器
        self.scorer_optimizer: Any | None = None
        if self.machine_scorer is not None:
            self.machine_scorer = self.machine_scorer.to(self.device)
            self.scorer_optimizer = Adam(
                self.machine_scorer.parameters(), lr=learning_rate, eps=1e-5
            )
