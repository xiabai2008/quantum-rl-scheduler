...
| 测试文件 | 94+ 个文件，3720 测试用例（pytest --co 实测，2026-08-07） |
...
| 多机器 MAPPO | 3机协同调度：总奖励 +86.3%（单机→多机，含资源扩展+协同）；同训练量收敛严格对比下协同优势 vs 独立PPO +4.0%（旧+84.6% p=0.019 为未收敛+训练量不均等的探索值，已废弃） |
