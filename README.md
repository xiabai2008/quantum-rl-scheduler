...
| 测试文件 | 94+ 个文件，3689 测试用例（pytest --collect-only 实测，2026-08-01） |
| 测试文件 | 94+ 个文件，3689 测试用例（pytest --collect-only 实测，2026-08-02） |
...
| 电路编译 AI | PPO编译Agent（探索性验证），公平对比v2（4×4 2D网格，同池配对60电路，Issue #451）；深电路(14-16q, 20电路)SWAP减少+33.3%（方向性优势，样本量不足），全60电路p=8.40e-01不显著，中+深40电路子集p=5.99e-01不显著；原76.4%为不公平对比已废弃 |
