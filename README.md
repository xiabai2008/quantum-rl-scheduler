...
| 方向 | 层级 | 核心成果 |
|:--|:--|:--|
| **AI赋能调度层** | PPO Agent 实时最优分流 | **+20.2%**（vs 真实 FCFS 量子路由基线，N=250，Welch t p=7.56e-12，95%CI [+14.3%,+26.7%]，8.5 基线诚实化；旧 +123.4% 为 vs Hybrid-Default 弱基线，已废弃） |
| **量子赋能AI** | 真机噪声训练注入（探索方向） | 真机噪声分布已注入 PPO 训练环境（N=50，100K）。奖励 +1.5%（p=0.584 不显著）；派单率 p=0.0203（效应量极小 99.6%→99.8%，且评估存在训练/测试分布混杂，需共同 holdout 交叉评估复核）。**诚实定位为方向性证据，不宣称鲁棒性已统计成立** |
...
- **4×4 2D网格耦合图**：匹配天衍-287真机nearest-neighbor拓扑，SWAP门减少62%（vs 线性链，拓扑消融口径，非编译Agent的 +52.1% N=200）
...
| 测试文件 | 94+ 个文件，3730 测试用例（pytest --co 实测，2026-08-19） |
| CI 强制覆盖率 | 80%（CI 实测通过，pyproject.toml `fail_under=80`；历史值 93.58% 随测试集扩充已变化，以 CI 报告为准） |
| 观测空间维度 | **16维**（新增串扰风险、任务到达率MA） |
| 动作空间 | **4维**（3维有效动作 + QUANTUM_QEM 误差缓释真机专用；仿真权威实验 N=250 中 QEM 分支语义退化为经典执行，见 env.py） |
| PPO-LSTM 训练 | 支持 `RecurrentPPO` 时序记忆（消融实验用），交付模型为标准PPO-MLP |
| 真机可用性验证 | 天衍-287 60/60任务完成（N=20 seeds×3策略，任务提交成功率100%） |
| PPO vs FCFS（仿真） | 综合奖励提升 20.2%（16维权威实验N=250，Welch t 检验 p=7.56e-12，rank-biserial=-0.3642 中效应） |
| PPO vs FCFS（真机参与率） | +108.5%（N=20 v3 权威, PPO=1632 vs FCFS=783, d=2.11, p=1.22e-07；注：混合评估环境，96步中仅1步涉及真机，"真机参与率"指每组提交1次真机任务，非性能来源；权威性能结论为仿真+20.2%） |
| 多机器 MAPPO | 3机协同调度：总奖励 +86.3%（单机→多机，含资源扩展+协同）；同训练量收敛严格对比下协同优势 vs 独立PPO +36.5%（N=20，配对 Wilcoxon p=0.024；旧+84.6% p=0.019 为未收敛+训练量不均等的探索值、旧+4.0% 为 N=10 口径，均已废弃） |
| 公平性调度 | Jain Fairness Index观测 + 等待时间偏差惩罚（#587/#588） |
| 电路编译 AI | PPO编译Agent（探索性验证），公平对比v2（4×4 2D网格，同池配对60电路，Issue #451）；深电路N=200规模化扩充（8.14，Issue #559）：SWAP减少+52.1%（Wilcoxon p=2.30e-10 高度显著，d_z=0.54，CI [+43.9%,+72.1%]）；全60电路p=8.40e-01不显著、浅/中电路无优势（诚实披露）；原76.4%为不公平对比已废弃 |
| 编译环境可配置 | 支持天衍-287(10x11网格)等多种拓扑（#594/#616） |
| VQE 行业场景 | 10分子×100任务，PPO +89.6% vs FCFS（14维模型，N=50 案例展示，非权威统计证据） |
| OR-Tools 对比 | 20/50/100/200任务，OR-Tools静态(全任务已知)求全局最优makespan，PPO动态在线决策；PPO flow time在静态集合口径下不占优（静态flow报告已诚实披露口径差异），优势在动态到达+实时决策场景 |
...
| 多机器调度 | 3机MAPPO协同：总提升+86.3%（含规模扩展+协同），同训练量收敛严格对比下协同优势 vs 独立PPO +36.5%（N=20，p=0.024；旧+84.6% p=0.019 为未收敛+训练量不均等的探索值、旧+4.0% 为 N=10 口径，均已废弃） | 已验证 |
...
| 实验 | 核心结论 |
|------|---------|
| 8策略对比 | PPO奖励1982.69 vs FCFS 1648.91，+20.2%（16维权威N=250，Welch t 检验 p=7.56e-12，rank-biserial=-0.3642 中效应） |
| 真机验证 | N=20 seeds, PPO=1632 vs FCFS=783 (+108.5%真机参与率验证，v3 权威 d=2.11 p=1.22e-07；注：混合环境，96步中仅1步涉及真机，"真机参与率"指每组提交1次真机任务，非性能来源；权威性能结论为仿真+20.2%)，60/60任务完成 |
| 六维消融 | D1算法+20.2% > D4多机+86.3%（含规模扩展+协同）> D5退火-5.6%（20seed权威，p=0.9430不显著，旧+6.4%已废弃）> D2状态+2.1%；MLP=LSTM收敛等价 |
| 电路编译（探索性） | 公平对比v2(4×4 2D网格, 60电路同池配对, Issue #451)：全60电路p=8.40e-01不显著；深电路N=200规模化扩充（Issue #559）：SWAP减少+52.1%（p=2.30e-10 高度显著）；2D网格vs线性链拓扑消融SWAP-62% |
| VQE行业 | 10分子×100任务, PPO +89.6% vs FCFS（14维模型，N=50 案例展示，非权威统计证据） |
| OR-Tools | CP-SAT静态(全任务已知)求全局最优，PPO动态在线实时决策；静态集合口径下PPO flow time不占优（口径差异见 ablation_ortools 报告），优势在动态到达+实时决策场景 |
