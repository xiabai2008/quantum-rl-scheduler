...
| 测试文件 | 94+ 个文件，3717 测试用例 + 21 benchmark = 3738（pytest --collect-only 实测，2026-08-05） |
...
| 多机器 MAPPO | 3机协同调度：总奖励 +86.3%（单机→多机，含资源扩展+协同）；同资源协同优势 vs 独立PPO +84.6%（p=0.019，10 seeds） |
...
├── tests/                    # 94+ 个测试文件，3717 用例 + 21 benchmark = 3738
├── tests/                    # 94+ 个测试文件，3717 用例 + 21 benchmark = 3738
│   └── benchmarks/           # 性能基准测试
...
| 多机器调度 | 3机MAPPO协同：总提升+86.3%（含规模扩展+协同），同资源协同优势 vs 独立PPO +84.6%（p=0.019） | 已验证 |
...
| 六维消融 | D1算法+20.2% > D4多机+86.3%（含规模扩展+协同）> D5退火+6.4%(不显著) > D2状态+2.1%；MLP=LSTM收敛等价 |
