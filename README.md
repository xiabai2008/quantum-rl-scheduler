...
| **量子赋能AI** | 真机噪声反馈优化鲁棒性 | N=25配对检验（Wilcoxon signed-rank p=2.98e-08, Cohen d_z=7.71, 奖励下降12.43%），数据源: quantum_noise_paired_canonical.md |
...
**量化目标：** 综合调度收益+123.4%（核心目标，已达成）；资源利用率+7.9%（p=0.0046，多目标权衡维度，高负载场景PPO显著优于FCFS）
...
| 测试文件 | 94 个文件，3400+ 测试用例（实测 3467，pytest --collect-only 2026-07-30） |
...
| PPO vs FCFS（仿真） | 综合奖励提升 123.4%（16维权威实验N=250，Welch t 检验 p=1.449e-66，Cohen's d=-2.1353） |
...
| 电路编译 AI | PPO替代SABRE，公平对比v2（4×4 2D网格，同池配对60电路，Issue #451）；深电路(14-16q, 20电路)SWAP减少+45.5%（方向性优势，样本量不足），全60电路p=5.95e-01不显著，中+深40电路子集p=2.56e-01不显著；原76.4%为不公平对比已废弃 |
...
| 电路编译 | 公平对比v2(4×4 2D网格, 60电路同池配对, Issue #451)：深电路(14-16q, 20电路)SWAP减少+45.5%（方向性优势，样本量不足），全60电路p=5.95e-01不显著，中+深40电路子集p=2.56e-01不显著；2D网格vs线性链拓扑消融SWAP-62% |
