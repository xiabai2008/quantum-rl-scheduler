...
> 本文件说明本项目**可提交的训练好的模型检查点**，用于保证评审在克隆仓库后能复现论文中的实验结果（PPO vs FCFS +20.2% 等）。
> Version: 9.1.0
> 最后更新：2026-07-31
...
| **MAPPO 多智能体** | `deliverable_models/mappo/mappo.pt` | 3 机协同 MAPPO（共享 Critic 投票仲裁），50K steps，seed=42 | ~323 KB | 严格同协议对比：4588.7 vs 独立PPO 4412.2（+4.0%）vs FCFS 923.9（+396.7%）。⚠️ FCFS 923.9 为固定 hybrid 基线（非真实 FCFS），+396.7% 仅作同 wrapper 严格对比，不对外作为权威性能提升；权威叙事以 multi_machine_comparison_report.md 为准（50K收敛 vs 同资源 FCFS +88.7%，协同优势 vs 独立PPO +84.6% p=0.019）。3 独立 PPO（independent_ppo_m0/1/2.zip，各 16K 步单机训练）随附 |
| **PPO编译优化Agent** | `deliverable_models/ppo_compilation_agent.zip` | 量子比特映射任务专用PPO Agent，4×4 2D网格训练200k steps | ~160 KB | 公平对比v2：全60电路+16.5%（p=8.40e-01不显著）；**深电路N=80扩充实验（2026-08-02, Issue #559）：+38.5%（Wilcoxon p=2.75e-02, t检验p=2.49e-03, 95%CI[+23.2%,+73.6%]），规模化效应：sabre>=10子集+63.2%（p=8.44e-08）——深电路显著优于SABRE** |
