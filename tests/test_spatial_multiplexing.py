...
from src.scheduler.env_types import ACTION_QUANTUM, QuantumMachine, Task
...
def test_spatial_multiplexing():
    ...
    task1 = Task(
        task_id="t1", task_type="quantum", qubit_count=3, execution_time=10.0, required_gates=["H"]
    )
    ...
    task2 = Task(
        task_id="t2", task_type="quantum", qubit_count=5, execution_time=10.0, required_gates=["H"]
    )
    ...
    task3 = Task(
        task_id="t3", task_type="quantum", qubit_count=3, execution_time=10.0, required_gates=["H"]
    )
...
def test_crosstalk_penalty():
    ...
    reward_single = compute_execution_reward(
        task,
        ACTION_QUANTUM,
        rng1,
        quantum_fidelity=0.98,
        quantum_available_ratio=1.0,
        crosstalk_penalty=0.0,
    )

    # Case with crosstalk penalty
    rng2 = np.random.default_rng(42)
    reward_concurrent = compute_execution_reward(
        task,
        ACTION_QUANTUM,
        rng2,
        quantum_fidelity=0.98,
        quantum_available_ratio=1.0,
        crosstalk_penalty=0.2,
    )
