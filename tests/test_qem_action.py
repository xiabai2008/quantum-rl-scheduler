...
from src.scheduler.env_types import ACTION_QUANTUM, ACTION_QUANTUM_QEM, Task, QuantumMachine
...
def test_qem_effects_direct():
    ...
    class MockEnv:
        def __init__(self):
            ...
            self._quantum = QuantumMachine(name="test_q_machine", total_qubits=10, fidelity=0.9, supported_gates=("H", "CZ", "M"), is_real=False)
            self._machines = [self._quantum] # For route_to_machine to access machine from env._machines
            ...
            self._machine_schedule_count = {m.name: 0 for m in self._machines} # For tracking machine schedules
    ...
    # Create a task
    task = Task(task_id="test_qem_task", task_type="quantum", qubit_count=1,
                priority=1, execution_time=10.0, required_gates=["H"])
    ...
    route_to_machine(mock_env, machine_instance_for_qem, task, rng, rl_action=ACTION_QUANTUM_QEM, is_qem=True)

    # Assertions for QEM effects
    # 1. Task execution time should be multiplied by 3.0
    assert abs(task.execution_time - original_exec_time * 3.0) < 1e-5, f"Expected {original_exec_time * 3.0}, Got {task.execution_time}"
    
    # 2. Machine fidelity should be improved (error rate halved)
    expected_fidelity = 1.0 - ((1.0 - original_fidelity) * 0.5)
    assert abs(machine_instance_for_qem.fidelity - expected_fidelity) < 1e-5, f"Expected {expected_fidelity}, Got {machine_instance_for_qem.fidelity}"
