...
class TestPlatformDetection(unittest.TestCase):
    ...
    def test_at_least_one_platform_detected(self) -> None:
...
class TestNormalizePath(unittest.TestCase):
    ...
    def test_backward_slash_path(self) -> None:
        """反斜杠路径应被规范化。"""
        result = normalize_path("a\\b\\c")
        self.assertEqual(result, os.path.normpath("a\\b\\c"))
...
class TestJoinPaths(unittest.TestCase):
    ...
    def test_join_with_empty_parts(self) -> None:
        """空段应被过滤掉。"""
        result = join_paths("a", "", "b", "", "c")
        self.assertEqual(result, os.path.join("a", "b", "c"))
    ...
    def test_join_single_part(self) -> None:
        """单段路径应原样返回。"""
        result = join_paths("a")
...
class TestSafeOpen(unittest.TestCase):
    ...
    def test_utf8_chinese_content(self) -> None:
    ...
    def test_mixed_separators_path(self) -> None:
        """应能处理包含混合分隔符的路径。"""
        with tempfile.TemporaryDirectory() as tmp:
            # 构造混合分隔符路径(需先创建子目录)
...
class TestGetLineEnding(unittest.TestCase):
    ...
    def test_returns_correct_ending(self) -> None:
        """应返回当前平台的正确换行符。"""
...
class TestEnsureWindowsEncoding(unittest.TestCase):
    ...
    def test_sets_pythonutf8_on_windows(self) -> None:
...
class TestRunSubprocess(unittest.TestCase):
    ...
    def test_failed_command_returncode(self) -> None:
        """失败的命令应返回非零退出码。"""
...
class TestKillProcessTree(unittest.TestCase):
    ...
    def test_kill_sleeping_child(self) -> None:
        """应能终止一个长时间运行的子进程。"""
        # 启动一个会运行较长时间的子进程
    ...
    def test_kill_invalid_pid_no_error(self) -> None:
        """无效 PID(0 和负数)不应抛出异常。"""
        # PID 0 和负数应被静默忽略
        kill_process_tree(0)
        kill_process_tree(-1)

    def test_kill_already_exited_process(self) -> None:
        """已退出的进程 PID 不应抛出异常。"""
        # 启动一个立即退出的子进程
        proc = subprocess.Popen(
