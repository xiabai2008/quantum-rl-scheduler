...
> **⚠️ v3 口径更新声明（2026-08-14）**：本报告中 Issue #244 的 N=10 多seed 数值（PPO=1736.32 vs FCFS=383.00, d=5.33, p=5.84e-07）为 v2 历史迭代数据，**已被 N=20 v3 权威取代**（8.14 扩样）：PPO=1632.26±326.49 vs FCFS=782.94±467.61, **d=2.11, p=1.22e-07, Bonferroni 显著**（v2 的 d=5.33 为小样本高估；v2 的 SJF vs FCFS 边缘支持 p=0.0316 在 N=20 下消失并反转 p=0.16，判定为小样本噪声）。对外一律使用 v3 数值（`results/reports/multiseed_real_machine_report_20seeds_v3.md` / `config/statistics.yaml real_machine_20seed_v3`）。下方 v2 细节仅保留作数据迭代历史追溯。
...
| **策略间对比（v3 权威）** | **PPO=1632.26±326.49 vs FCFS=782.94±467.61, Cohen's d=2.11, p=1.22e-07, Bonferroni 显著** | **N=20 seeds × 3 策略（8.14 扩样；v2 N=10 的 d=5.33 为小样本高估，见下方历史记录）** |
...
> **v3 权威（8.14 扩样）**：20 seeds × 3 策略 = 60 次真机任务全部完成，20/20 seeds 真机参与率 > 0，
> PPO vs FCFS Cohen's d=2.11（大效应），p=1.22e-07（Bonferroni 显著；v2 N=10 的 d=5.33 为小样本高估已回归正常化）。
> 当前权威性能提升结论仍由仿真实验支撑（16维交付模型 PPO vs 真实 FCFS +20.2%, N=250, Welch t p=7.56e-12, 8.5 基线诚实化，见 `config/statistics.yaml`；旧14维口径 +88.3% 已废弃），
...
| 项 | 值 |
|:--|:--|
| 平台 | 天衍-287（287 总比特（105 数据 + 182 耦合）超导处理器） |
...
- 284 次真机调用 100% 成功，287 总比特（105 数据 + 182 耦合）超导处理器
...
性能提升结论由仿真实验支撑：16维交付模型 PPO vs 真实 FCFS +20.2%（N=250, Welch t p=7.56e-12, 8.5 基线诚实化，旧14维口径 +88.3% 已废弃）。
