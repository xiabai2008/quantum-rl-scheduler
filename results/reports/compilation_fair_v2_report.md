...
|:--|:--:|:--:|:--:|
| 浅电路 (20 电路) | 1.35 ± 2.72 | 2.00 ± 1.84 | -48.1% |
| 中电路 (20 电路) | 8.65 ± 12.97 | 8.75 ± 2.57 | -1.2% |
| 深电路 (20 电路) | 24.95 ± 27.52 | 13.60 ± 3.44 | +45.5% |
...
|:--|:--|
| W 统计量 | 838 |
| p 值（单侧 PPO < SABRE） | 6.28e-01 |
| 配对数 | 60 |
| rank-biserial 效应量 | -0.050 |
| Cohen's d（配对） | 0.196 |
| 显著性 (α=0.05) | [FAIL] 不显著 |

**结论**: p = 6.28e-01，未达到 0.05 显著性水平，PPO 平均 SWAP 数较低但差异不显著。

### 提升百分比 95% CI

| 方法 | CI |
|:--|:--|
| Bootstrap (2000 次) | [-9.9%, 50.6%] |
...
|:--|:--|
| Wilcoxon W | 345 |
| p 值（单侧 PPO < SABRE） | 2.65e-01 |
| rank-biserial 效应量 | 0.115 |
| Cohen's d（配对） | 0.259 |
| Bootstrap 95% CI | [-5.8%, 52.7%] |
| 显著性 (α=0.05) | [FAIL] 不显著 |

**子集结论**: p = 2.65e-01，子集检验未达到 0.05 显著性水平，PPO 平均较低但差异不显著。
...
| 本公平对比 v2（全 60 电路） | 11.65 | 8.12 | +30.3% | [PASS] 同池配对 | [PASS] seed=42 | p=6.28e-01 |
| 本公平对比 v2（中+深 40 电路） | 16.80 | 11.18 | +33.5% | [PASS] 同池配对 | [PASS] seed=42 | p=2.65e-01 |
...
全60电路平均变化 +30.3%（p=6.28e-01），
中+深电路子集变化 +33.5%（p=2.65e-01），
...
3. **当前模型评测结果**：全60电路 SABRE avg=11.65 SWAP，
   PPO avg=8.12 SWAP，变化 +30.3%
   （p=6.28e-01）；中+深子集变化 +33.5%（p=2.65e-01）。
4. **统计结论**：PPO平均SWAP数低于SABRE但差异未达统计显著性(p>0.05)，深电路类别改进45.5%，提示PPO在复杂电路上有优势，提示需更大规模训练验证。
