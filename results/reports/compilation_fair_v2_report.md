...
> **方法论声明**：本对比为 **PPO 端到端编译（布局+路由）** vs **SABRE 端到端编译（布局+路由）** 的整体性能对比，
> 非纯粹 SWAP 路由算法对比。SABRE 使用 `SabreLayout` 自动搜索初始映射，PPO 通过 `QuantumCompilationEnv`
> 内部映射机制，两者初始布局策略不同。对比结果中混入了"布局策略差异"和"路由策略差异"两个变量。
> 该对比在应用层面（端到端编译性能）仍有意义，但不应解读为纯粹的路由算法对比。
...
| 类别 | SABRE avg ± std | PPO avg ± std | 改进率 | Wilcoxon p (单侧) | 效应量 r | 显著性 |
|:--|:--:|:--:|:--:|:--:|:--:|:--:|
| 浅电路 (n=20) | 1.25 ± 2.63 | 1.90 ± 1.89 | -52.0% | 0.9015 | 0.31 | ❌ 不显著 |
| 中电路 (n=20) | 7.20 ± 11.44 | 8.65 ± 2.76 | -20.1% | 0.9196 | 0.31 | ❌ 不显著 |
| 深电路 (n=20) | 20.20 ± 24.17 | 13.60 ± 3.56 | +32.7% | 0.2876 | 0.13 | ❌ 不显著 |

### 深电路子集单独统计检验（Issue #564 补充）

> **统计局限性声明**：深电路 "+32.7%" 基于 n=20 小样本，Wilcoxon signed-rank 单侧检验 p=0.2876，
> 效应量 r=0.13（small），Bootstrap 95% CI 为 [-28.8%, 55.9%]（跨越零），**不具统计显著性**。
> 该数字仅作为趋势性观察，不作为核心结论。

| 统计量 | 深电路 (n=20) |
|:--|:--|
| Wilcoxon W 统计量 | 120.0 |
| p 值（单侧 SABRE > PPO） | 0.2876 |
| p 值（双侧） | 0.5752 |
| 效应量 r | 0.13 (small) |
| Bootstrap 95% CI (改进%) | [-28.8%, 55.9%] |
| 显著性 (α=0.05) | ❌ 不显著 |
...
|:--|:--|
| W 统计量 | 963 |
| p 值（单侧 PPO < SABRE） | 8.61e-01 |
| 配对数 | 60 |
| rank-biserial 效应量 | -0.165 |
| Cohen's d（配对） | 0.097 |
| 显著性 (α=0.05) | ❌ 不显著 |

**结论**: p = 8.61e-01，未达到 0.05 显著性水平，PPO 平均 SWAP 数较低但差异不显著。

### 提升百分比 95% CI

| 方法 | CI |
|:--|:--|
| Bootstrap (2000 次) | [-36.1%, 41.8%] |
...
|:--|:--|
| Wilcoxon W | 440 |
| p 值（单侧 PPO < SABRE） | 6.54e-01 |
| rank-biserial 效应量 | -0.072 |
| Cohen's d（配对） | 0.136 |
| Bootstrap 95% CI | [-32.2%, 44.2%] |
| 显著性 (α=0.05) | ❌ 不显著 |

**子集结论**: p = 6.54e-01，子集检验未达到 0.05 显著性水平，PPO 平均较低但差异不显著。
...
| 本公平对比 v2（全 60 电路） | 9.55 | 8.05 | +15.7% | ✅ 同池配对 | ✅ seed=42 | p=8.61e-01 |
| 本公平对比 v2（中+深 40 电路） | 13.70 | 11.12 | +18.8% | ✅ 同池配对 | ✅ seed=42 | p=6.54e-01 |
...
全60电路平均变化 +15.7%（p=8.61e-01），
中+深电路子集变化 +18.8%（p=6.54e-01），
...
3. **当前模型评测结果**：全60电路 SABRE avg=9.55 SWAP，
   ...
   （p=8.61e-01）；中+深子集变化 +18.8%（p=6.54e-01）。
4. **统计结论**：PPO平均SWAP数低于SABRE但差异未达统计显著性(p>0.05)。
   深电路类别改进+32.7%（n=20，Wilcoxon p=0.2876，效应量r=0.13 small，Bootstrap 95% CI [-28.8%, 55.9%]跨越零），
   **不具统计显著性**，仅作为趋势性观察，提示PPO在复杂电路上可能有优势但需更大规模验证。
