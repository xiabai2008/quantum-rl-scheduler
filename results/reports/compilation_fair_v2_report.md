...
> **⚠️ 冻结横幅**：本报告深电路 +33.3%（N=20）为早期样本，已被深电路规模化扩充实验取代（N=80 方向性 → 8.14 N=200 权威 +52.1% p=2.30e-10；全 60 电路 p=8.40e-01 不显著，浅/中电路无优势，诚实披露），权威口径见 `results/reports/compilation_deep_scale_report.md` 与 `config/statistics.yaml` compilation_deep_scale_n200。
...
|:--|:--:|:--:|:--:|
| 浅电路 (20 电路) | 1.30 ± 2.64 | 2.00 ± 1.84 | -53.8% |
| 中电路 (20 电路) | 7.45 ± 11.71 | 8.75 ± 2.57 | -17.4% |
| 深电路 (20 电路) | 20.40 ± 23.95 | 13.60 ± 3.44 | +33.3% |
...
|:--|:--|
| W 统计量 | 920 |
| p 值（单侧 PPO < SABRE） | 8.40e-01 |
| 配对数 | 60 |
| rank-biserial 效应量 | -0.153 |
| Cohen's d（配对） | 0.103 |
| 显著性 (α=0.05) | [FAIL] 不显著 |

**结论**: p = 8.40e-01，未达到 0.05 显著性水平，PPO 平均 SWAP 数较低但差异不显著。

### 提升百分比 95% CI

| 方法 | CI |
|:--|:--|
| Bootstrap (2000 次) | [-33.8%, 41.8%] |
| 逐电路改进率百分位 | [-1357.5%, 100.0%] |
| 逐电路改进率均值 | -325.4% |
| 逐电路改进率中位数 | -100.0% |
...
|:--|:--|
| Wilcoxon W | 408 |
| p 值（单侧 PPO < SABRE） | 5.99e-01 |
| rank-biserial 效应量 | -0.046 |
| Cohen's d（配对） | 0.146 |
| Bootstrap 95% CI | [-29.1%, 44.5%] |
| 显著性 (α=0.05) | [FAIL] 不显著 |

**子集结论**: p = 5.99e-01，子集检验未达到 0.05 显著性水平，PPO 平均较低但差异不显著。
...
| 本公平对比 v2（全 60 电路） | 9.72 | 8.12 | +16.5% | [PASS] 同池配对 | [PASS] seed=42 | p=8.40e-01 |
| 本公平对比 v2（中+深 40 电路） | 13.93 | 11.18 | +19.7% | [PASS] 同池配对 | [PASS] seed=42 | p=5.99e-01 |
...
全60电路平均变化 +16.5%（p=8.40e-01），
中+深电路子集变化 +19.7%（p=5.99e-01），
...
3. **当前模型评测结果**：全60电路 SABRE avg=9.72 SWAP，
   PPO avg=8.12 SWAP，变化 +16.5%
   （p=8.40e-01）；中+深子集变化 +19.7%（p=5.99e-01）。
4. **统计结论**：PPO平均SWAP数低于SABRE但差异未达统计显著性(p>0.05)，深电路类别改进33.3%，提示PPO在复杂电路上有优势，提示需更大规模训练验证。
