...
> **实验配置**: 2 seeds × 2 episodes × 2 条件 = 8 次独立运行
...
> **显著性水平**: α = 0.05
> **生成时间**: 2026-08-03T20:02:39.845734+08:00
...
|:--|:--:|:--:|
| 配对样本数 N | 2 | 2 |
| seed 平均奖励 均值 ± 标准差 | 1803.9856 ± 613.7684 | 1582.4827 ± 551.6254 |
| 全部 episode 奖励 均值 | 1803.9856 | 1582.4827 |

| 统计检验 | 值 | 说明 |
|:--|:--:|:--|
| Wilcoxon W 统计量 | 3.0000 | 正秩和（Standard > DistNoise 的秩次） |
| p 值 | 0.25 | 单侧检验 H1: Standard > DistNoise |
| 显著性 (α=0.05) | 不显著 | 不能拒绝 H0，噪声影响不显著 |
| Cohen's d_z | 3.5644 | 配对效应量（>0.8 大效应） |
| 95% CI (配对差值) | [-336.8300, 779.8359] | 均值差的 95% 置信区间 |
| 事后功效 | 0.4886 | 当前样本量下的检验力（目标≥0.8） |
| 噪声奖励变化% | -12.28% | 负值=噪声导致奖励下降 |
| 变化% 95% CI | [-38.87%, +25.24%] | Bootstrap 95% CI |

## 四、结论

**统计结论**：在 α=0.05 显著性水平下，Wilcoxon signed-rank 检验表明
真机噪声分布对 PPO 策略奖励**影响不显著**（p=0.25）。
配对效应量 Cohen's d_z=3.5644，事后功效=0.4886。

**解读**：
- 噪声奖励变化 -12.28%（95% CI: [-38.87%, +25.24%]）
- 当前样本量（N=2）下功效 不足,
需扩大样本量进一步验证
...
---
*量子赋能AI v3 证据链: tianyan-287 10seeds MBS → 噪声分布 → PPO 配对检验 (N=2)*