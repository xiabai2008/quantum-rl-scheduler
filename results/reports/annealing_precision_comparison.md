...
> **⚠️ 历史报告冻结声明（8.14 第五轮审查）**：本报告为 **2026-07-26 的 5seed 早期实验**。
> 文中作为当前基线的 +6.4%（p=0.19）已被 **20seed 权威实验取代**：退火权威方向为
> **-5.6%（p=0.9430 不显著）**，结论为"量子启发式退火（经典模拟退火）无显著收益，已降级为
> 探索性功能、默认关闭"（权威口径：`config/statistics.yaml` annealing 段；
> 旧 5seed +6.4% 已废弃）。本报告下方"退火效果 +6.4%（p=0.19 不显著）"等表述仅代表
> 2026-07 时点探索性观察，不得作为对外性能基准引用。
...
当前默认 `anneal_qubits=16`（`n_bits_per_weight=4`，1 符号位 + 3 数值位），退火效果 +6.4%（p=0.19 不显著，5seed 旧口径，已被 20seed 权威 -5.6% p=0.9430 取代）。需验证：降低编码精度是否进一步削弱退火效果，从而证明「退火效果不显著源于编码精度不足而非方法论缺陷」。
...
- 若所有组均不显著：保持现有结论（退火 +6.4%, p=0.19，5seed 旧口径已废弃，权威为 20seed -5.6% p=0.9430），在 `docs/annealing_significance-defense.md` 中补充「精度对比实验已排除编码精度问题」的论证。
...
| 文档 | 关系 |
|:--|:--|
| `results/reports/ablation_report.md` | D5 退火消融实验（+6.4%, p=0.19，5seed 旧口径已废弃；权威见 20seed -5.6% p=0.9430） |
