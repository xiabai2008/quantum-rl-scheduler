...
> **⚠️ 历史报告冻结声明（2026-07-29）**：本报告基于 14 维旧模型（ppo_best_model_14dim.zip，已删除）产出，文中引用的 +88.3%、p=1.032e-42 为旧口径，**已废弃**。现行权威口径为 16 维交付模型：PPO vs FCFS **+123.4%**（N=250, Welch t p=1.449e-66），见 `config/statistics.yaml`。敏感性趋势结论（量子占比影响方向）仍可参考，但具体数字禁止直接引用。
