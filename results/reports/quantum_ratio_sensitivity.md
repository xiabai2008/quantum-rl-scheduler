...
> **⚠️ 历史报告冻结声明（2026-07-29）**：本报告基于 14 维旧模型（ppo_best_model_14dim.zip，已删除）产出，文中引用的 +88.3%、p=1.032e-42 为旧口径，**已废弃**。现行权威口径为 16 维交付模型：PPO vs 真实 FCFS **+20.2%**（N=250, Welch t p=7.56e-12, 8.5 基线诚实化；旧 +123.4% 为 vs Hybrid-Default 弱基线），见 `config/statistics.yaml`。敏感性趋势结论（量子占比影响方向）仍可参考，但具体数字禁止直接引用。
...
已有历史数字（14 维旧口径 PPO +88.3%, p=1.032e-42，已废弃）仅在量子占比 70% 这一个点测得；现行权威口径为 16 维交付模型 +20.2%（N=250, Welch t p=7.56e-12, 8.5 基线诚实化 vs 真实 FCFS）。
...
- **与历史数字对比**: 70% 量子占比下测得提升 +91.6%（历史 14 维口径 +88.3%，已废弃；差异来源于 seed 数/训练策略/模型版本）
