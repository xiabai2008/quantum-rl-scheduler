...
> **显著性水平**: α = 0.05
> **生成时间**: 2026-07-29T09:51:41.227809+08:00
...
|:--|:--:|:--:|
| 配对样本数 N | 25 | 25 |
| seed 平均奖励 均值 ± 标准差 | nan ± nan | nan ± nan |
| 全部 episode 奖励 均值 | nan | nan |

| 统计检验 | 值 | 说明 |
|:--|:--:|:--|
| Wilcoxon W 统计量 | nan | 正秩和（Standard > DistNoise 的秩次） |
| p 值 | nan | 单侧检验 H1: Standard > DistNoise |
| 显著性 (α=0.05) | 不显著 | 不能拒绝 H0，噪声影响不显著 |
| Cohen's d_z | nan | 配对效应量（>0.8 大效应） |
| 95% CI (配对差值) | [nan, nan] | 均值差的 95% 置信区间 |
| 事后功效 | nan | 当前样本量下的检验力（目标≥0.8） |
| 噪声奖励变化% | +nan% | 负值=噪声导致奖励下降 |
| 变化% 95% CI | [+nan%, +nan%] | Bootstrap 95% CI |

## 四、结论

**统计结论**：在 α=0.05 显著性水平下，Wilcoxon signed-rank 检验表明
真机噪声分布对 PPO 策略奖励**影响不显著**（p=nan）。
配对效应量 Cohen's d_z=nan，事后功效=nan。

**解读**：
- 噪声奖励变化 +nan%（95% CI: [+nan%, +nan%]）
- 当前样本量（N=25）下功效 不足,
需扩大样本量进一步验证
