...
**最后更新**：2026-07-24（Issue #128：真机验证结论边界文档完成；真机API密钥验证通过，tianyan176 H门任务成功执行，14后端可见11 running）
...
> - ⚠️ **性能验证（不充分）**：mixed_real vs simulation p=0.344不显著（N=5, 需N≥18）；多seed策略对比p=6.83e-04显著，但真机任务运行失败，奖励由仿真环境计算
> - **性能提升结论由仿真实验支撑**：PPO vs FCFS +88.3%（N=250, p=3.04e-11, d=-1.70）
>
...
> **⚠️ 边界说明**：真机任务运行失败，以下奖励由仿真环境计算，反映策略鲁棒性而非真机性能差异
