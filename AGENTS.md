...
├── tests/                        # 测试（94+ 文件，3730 用例，pytest --co 实测 2026-08-19）
...
- 测试文件：5 → 76（+71个专用测试模块）
- 测试用例：100+ → 3751（全量收集，含 21 benchmark）；主套件口径 3730（排除 benchmark，doc-sync 基准）；pytest --collect-only 实测，2026-08-19（历史口径随测试集扩充已多次更新，以当前实测为准）
