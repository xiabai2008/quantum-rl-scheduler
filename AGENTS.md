...
├── tests/                        # 测试（94+ 文件，3723 用例，pytest --co 实测 2026-08-07）
...
- 测试文件：5 → 76（+71个专用测试模块）
- 测试用例：100+ → 3744（全量收集，含 21 benchmark）；主套件口径 3723（排除 benchmark，doc-sync 基准）；pytest --collect-only 实测，2026-08-08（历史口径随测试集扩充已多次更新，以当前实测为准）
